...
-type(amqp_method_field_name() ::
       ( host | ticket | multiple | ticket
       | exchange | no_local | requeue | routing_key
       | consumer_tag | prefetch_size | string_op | routing_key
       | delivery_tag | frame_max | ticket | message_count
       | consume_rate | queue | challenge | identifier
       | ticket | if_unused | consumer_tag | reply_text
       | reply_text | reply_text | requeue | nowait
       | exchange | ticket | heartbeat | delivery_tag
       | known_hosts | string_1 | integer_2 | delivery_tag
       | exchange | locales | delivery_tag | routing_key
       | capabilities | reply_text | internal | arguments
       | identifier | consumer_tag | exchange | active
       | queue | mechanisms | auto_delete | exchange
       | nowait | routing_key | consumer_tag | nowait
       | routing_key | reply_code | prefetch_count | prefetch_size
       | integer_1 | queue | queue | table
       | passive | auto_delete | version_minor | content_size
       | if_unused | queue | exchange | exchange
       | string_2 | if_empty | identifier | nowait
       | consumer_tag | ticket | nowait | ticket
       | consumer_tag | routing_key | nowait | string_result
       | method_id | locale | routing_key | integer_4
       | redelivered | reply_code | consumer_tag | out_of_band
       | class_id | ticket | nowait | durable
       | delivery_tag | prefetch_count | consumer_tag | insist
       | durable | consumer_tag | routing_key | exclusive
       | exchange | channel_max | exclusive | integer_result
       | global | immediate | exchange | consumer_tag
       | content_checksum | no_ack | operation | write
       | queue | integer_op | server_properties | message_count
       | consumer_tag | routing_key | reply_text | response
       | no_ack | consumer_count | exclusive | queue
       | reply_code | consumer_tag | queue | multiple
       | integer_3 | ticket | exclusive | requeue
       | exchange | frame_max | exchange | type
       | queue | delivery_tag | read | redelivered
       | nowait | exchange | active | nowait
       | details | mandatory | nowait | immediate
       | no_ack | client_properties | class_id | delivery_tag
       | known_hosts | reply_code | cluster_id | prefetch_count
       | no_local | mandatory | heartbeat | response
       | consumer_tag | meta_data | passive | message_count
       | result | mechanism | ticket | reply_text
       | consumer_tag | version_major | exchange | mandatory
       | arguments | routing_key | exclusive | result
       | ticket | dtx_identifier | routing_key | method_id
       | message_count | queue | reply_code | exchange
       | global | prefetch_size | delivery_tag | nowait
       | ticket | immediate | channel_max | arguments
       | consumer_tag | ticket | redelivered | requeue
       | realm | virtual_host | operation | ticket
       | staged_size | global | reply_code | queue
       | active | nowait | arguments | passive
       | ticket | no_local )).
