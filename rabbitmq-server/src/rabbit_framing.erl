...
-type(amqp_method_field_name() ::
       ( nowait | method_id | ticket | consumer_tag
       | reply_text | routing_key | ticket | redelivered
       | delivery_tag | exclusive | channel_max | heartbeat
       | nowait | ticket | delivery_tag | consumer_tag
       | integer_result | version_minor | locales | consumer_tag
       | routing_key | ticket | delivery_tag | routing_key
       | exchange | exclusive | queue | virtual_host
       | nowait | global | immediate | arguments
       | consumer_tag | content_checksum | ticket | class_id
       | integer_op | prefetch_size | server_properties | consumer_tag
       | out_of_band | exchange | reply_code | routing_key
       | realm | frame_max | routing_key | queue
       | auto_delete | reply_code | if_unused | queue
       | integer_1 | ticket | queue | reply_text
       | passive | ticket | exchange | exchange
       | exchange | string_result | ticket | reply_code
       | known_hosts | consumer_tag | redelivered | locale
       | nowait | no_local | consumer_tag | if_unused
       | prefetch_count | mandatory | internal | class_id
       | exchange | integer_2 | content_size | delivery_tag
       | no_ack | mechanisms | durable | string_2
       | cluster_id | nowait | prefetch_count | string_1
       | routing_key | exclusive | consumer_tag | queue
       | queue | ticket | consumer_tag | passive
       | prefetch_count | consumer_tag | reply_text | consumer_tag
       | exchange | nowait | response | routing_key
       | consumer_tag | nowait | result | message_count
       | routing_key | challenge | queue | nowait
       | reply_code | method_id | exclusive | delivery_tag
       | exclusive | nowait | consume_rate | immediate
       | message_count | queue | message_count | requeue
       | reply_code | integer_4 | redelivered | active
       | operation | multiple | global | staged_size
       | channel_max | no_ack | consumer_tag | no_local
       | nowait | exchange | mandatory | delivery_tag
       | ticket | write | reply_code | ticket
       | message_count | exchange | no_local | routing_key
       | arguments | reply_text | type | prefetch_size
       | routing_key | consumer_count | operation | ticket
       | reply_text | ticket | ticket | no_ack
       | reply_text | nowait | known_hosts | arguments
       | requeue | frame_max | global | response
       | read | dtx_identifier | delivery_tag | client_properties
       | string_op | exchange | insist | exchange
       | nowait | delivery_tag | details | immediate
       | arguments | identifier | auto_delete | consumer_tag
       | active | durable | routing_key | exchange
       | queue | prefetch_size | if_empty | heartbeat
       | active | passive | requeue | requeue
       | queue | mechanism | host | table
       | exchange | version_major | queue | exchange
       | multiple | mandatory | capabilities | meta_data
       | identifier | identifier | result | ticket
       | integer_3 | consumer_tag )).
