...
-type(amqp_method_field_name() ::
       ...
       | nowait | consumer_tag | queue | routing_key
       | delivery_tag | virtual_host | ticket | frame_max
       | routing_key | identifier | string_2 | ticket
       | nowait | consumer_tag | reply_text | message_count
       | ticket | requeue | nowait | reply_text
       | queue | capabilities | delivery_tag | consumer_tag
       | no_local | routing_key | reply_code | delivery_tag
       | queue | method_id | delivery_tag | multiple
       | if_unused | exchange | exclusive | heartbeat
       | ticket | queue | result | arguments
       | consumer_tag | internal | ticket | exchange
       | passive | routing_key | arguments | nowait
       | class_id | active | integer_1 | exchange
       | dtx_identifier | queue | prefetch_size | table
       | queue | exchange | response | content_size
       | routing_key | passive | queue | exchange
       | exchange | class_id | identifier | insist
       | consumer_tag | nowait | ticket | exchange
       | consumer_tag | challenge | reply_text | multiple
       | locales | auto_delete | routing_key | redelivered
       | method_id | reply_code | reply_code | mechanisms
       | nowait | message_count | delivery_tag | prefetch_count
       | consumer_tag | durable | routing_key | consumer_tag
       | routing_key | requeue | exclusive | message_count
       | locale | global | immediate | arguments
       | integer_2 | content_checksum | no_ack | message_count
       | consumer_tag | integer_op | global | prefetch_size
       | write | consumer_tag | routing_key | ticket
       | queue | ticket | reply_text | consumer_count
       | server_properties | reply_code | queue | queue
       | reply_text | ticket | exchange | exclusive
       | requeue | routing_key | known_hosts | exchange
       | frame_max | if_empty | response | redelivered
       | consume_rate | read | prefetch_size | nowait
       | string_result | consumer_tag | nowait | mandatory
       | consumer_tag | operation | nowait | details
       | no_ack | integer_3 | no_local | delivery_tag
       | auto_delete | version_minor | cluster_id | prefetch_count
       | no_local | mandatory | prefetch_count | active
       | heartbeat | consumer_tag | passive | if_unused
       | string_op | reply_text | consumer_tag | mechanism
       | exchange | staged_size | meta_data | version_major
       | consumer_tag | routing_key | integer_4 | result
       | identifier | queue | active | type
       | ticket | channel_max | nowait | reply_code
       | exchange | exclusive | mandatory | delivery_tag
       | exclusive | host | immediate | exchange
       | ticket | channel_max | integer_result | durable
       | redelivered | requeue | ticket | client_properties
       | operation | known_hosts | global | out_of_band
       | reply_code | nowait | no_ack | realm
       | ticket | delivery_tag | ticket | immediate
       | string_1 | nowait )).
