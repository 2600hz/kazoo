...
-type(amqp_method_field_name() ::
       ( arguments | message_count | integer_1 | requeue
       | queue | reply_text | table | routing_key
       | response | content_size | virtual_host | queue
       | exchange | string_op | immediate | auto_delete
       | routing_key | routing_key | nowait | consumer_tag
       | server_properties | ticket | exchange | consumer_tag
       | reply_text | integer_4 | ticket | multiple
       | active | capabilities | routing_key | redelivered
       | no_local | reply_code | known_hosts | nowait
       | nowait | result | delivery_tag | prefetch_count
       | exchange | integer_result | consumer_tag | if_unused
       | exchange | consumer_tag | routing_key | heartbeat
       | operation | exclusive | staged_size | consumer_tag
       | internal | string_result | global | immediate
       | details | passive | integer_2 | content_checksum
       | string_1 | no_ack | class_id | active
       | integer_op | prefetch_size | exchange | consumer_tag
       | routing_key | queue | exchange | passive
       | class_id | reply_code | queue | queue
       | nowait | ticket | nowait | method_id
       | requeue | challenge | exchange | exchange
       | locales | queue | ticket | redelivered
       | queue | routing_key | mechanisms | consumer_tag
       | nowait | consumer_tag | message_count | mandatory
       | prefetch_size | nowait | durable | reply_code
       | mandatory | dtx_identifier | multiple | delivery_tag
       | message_count | locale | cluster_id | arguments
       | prefetch_count | no_local | reply_text | exchange
       | consumer_tag | consumer_tag | exchange | write
       | consumer_tag | queue | ticket | reply_text
       | consumer_tag | nowait | exchange | meta_data
       | consumer_count | message_count | routing_key | result
       | reply_text | routing_key | client_properties | exclusive
       | channel_max | known_hosts | reply_code | exchange
       | exclusive | frame_max | if_empty | delivery_tag
       | exclusive | response | global | immediate
       | read | prefetch_size | consume_rate | nowait
       | redelivered | requeue | arguments | no_ack
       | operation | no_local | global | out_of_band
       | auto_delete | realm | nowait | delivery_tag
       | ticket | prefetch_count | queue | ticket
       | exchange | heartbeat | consumer_tag | passive
       | if_unused | ticket | delivery_tag | mechanism
       | ticket | version_major | consumer_tag | identifier
       | frame_max | ticket | consumer_tag | reply_text
       | active | type | requeue | nowait
       | string_2 | method_id | mandatory | delivery_tag
       | routing_key | host | identifier | reply_code
       | delivery_tag | ticket | channel_max | delivery_tag
       | durable | ticket | queue | identifier
       | exclusive | version_minor | ticket | queue
       | reply_code | insist | integer_3 | ticket
       | exchange | ticket | routing_key | no_ack
       | arguments | nowait )).
