...
With `{diff_output}` constructed, invoke the `evo-review-adversarial-general` skill. If possible, use information asymmetry: invoke the skill in a separate subagent or process with read access to the project, but no context except the `{diff_output}`.
...
With findings in hand, read fully and follow: `{project-root}/_evo/bmm/workflows/evo-quick-flow/quick-dev/steps/step-06-resolve-findings.md` for user to choose resolution approach.
