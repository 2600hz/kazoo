...
load_merge(DocId, #cb_context{doc=DataJObj}=Context) -> load_merge(DocId, DataJObj, Context).

load_merge(DocId, DataJObj, #cb_context{db_name=DbName
                                        ,load_merge_bypass='undefined'
                                       }=Context) ->
...
save(#cb_context{}=Context) -> save(Context, []).
...
save_attachment(DocId, AName, Contents, Context) -> save_attachment(DocId, AName, Contents, Context, []).
