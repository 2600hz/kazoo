...
%% @end
%%--------------------------------------------------------------------
handle_info({binding_fired, Pid, <<"v1_resource.allowed_methods.agents">>, Payload}, State) ->
    spawn(fun() ->
...
handle_info({binding_fired, Pid, <<"v1_resource.resource_exists.agents">>, Payload}, State) ->
    spawn(fun() ->
...
handle_info({binding_fired, Pid, <<"v1_resource.validate.agents">>, [RD, Context | Params]}, State) ->
    spawn(fun() ->
...
handle_info({binding_fired, Pid, <<"v1_resource.execute.post.agents">>, [RD, Context | Params]}, State) ->
    spawn(fun() ->
                  _ = crossbar_util:put_reqid(Context),
...
handle_info({binding_fired, Pid, <<"v1_resource.execute.put.agents">>, [RD, Context | Params]}, State) ->
    spawn(fun() ->
                  _ = crossbar_util:put_reqid(Context),
                  Context1 = crossbar_doc:save(Context),
                  Pid ! {binding_result, true, [RD, Context1, Params]}
...
handle_info({binding_fired, Pid, <<"v1_resource.execute.delete.agents">>, [RD, Context | Params]}, State) ->
    spawn(fun() ->
                  _ = crossbar_util:put_reqid(Context),
...
bind_to_crossbar() ->
    _ = crossbar_bindings:bind(<<"v1_resource.allowed_methods.agents">>),
    _ = crossbar_bindings:bind(<<"v1_resource.resource_exists.agents">>),
    _ = crossbar_bindings:bind(<<"v1_resource.validate.agents">>),
    crossbar_bindings:bind(<<"v1_resource.execute.#.agents">>).
...
%% /v1/accounts/AID/agents
allowed_methods([]) ->
    {true, ['GET', 'PUT']};
%% /v1/accounts/AID/agents/AGID
allowed_methods([_]) ->
