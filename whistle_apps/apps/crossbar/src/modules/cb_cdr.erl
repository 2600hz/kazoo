...
load_cdr_summary(Context, QueryString) ->
    ?LOG(" >> QueryString =  ~p", [QueryString]),
    case QueryString of 
        [_|_] -> do_filter_summary(Context, QueryString);
        _ -> crossbar_doc:load_view(?CB_LIST, [], Context, fun normalize_view_results/2)
    end.

-spec(do_filter_summary/2 :: (Context :: #cb_context{}, QueryString :: list()) -> #cb_context{}).
do_filter_summary(Context, QueryString) ->
    crossbar_doc:load_view(?CB_LIST, [{<<"include_docs">>, true}], Context, fun normalize_view_results/2).
%%--------------------------------------------------------------------
%% @private
%% @doc
