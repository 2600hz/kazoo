...
-spec store/1 :: (CDR) -> tuple(ok, wh_json:json_object()) | tuple(error, atom()) when
      CDR :: wh_json:json_object().
store(CDR) ->
...
-spec(store_cdr/3 :: (CDR :: wh_json:json_object(), Flags :: #route_flags{}, DB :: binary()) -> no_return()).
...
-spec find_route_used/3 :: (ne_binary(), ne_binary(), wh_json:json_object() | wh_json:json_objects()) -> wh_json:json_object().
find_route_used(<<"outbound">>, ToUri, Routes) when is_list(Routes) ->
    ...
    lists:foldl(fun(RouteJObj, Acc) ->
                ...
                end, wh_json:new(), Routes);
find_route_used(<<"inbound">>, _, _) -> wh_json:new();
find_route_used(Dir, To, Route) ->
    true = wh_json:is_json_object(Route),
    find_route_used(Dir, To, [Route]).

-spec fetch_cdr/2 :: (ne_binary(), ne_binary()) -> {'error', 'not_found'} | {'ok', wh_json:json_object()}.
fetch_cdr(CallID, DB) ->
