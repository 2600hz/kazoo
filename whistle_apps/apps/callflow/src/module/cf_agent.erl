...
-record(prompts, {
          pin_prompt = <<"/system_media/agent_pin">>
         ,welcome_prompt = <<"/system_media/agent_welcome">>
         }).
...
find_agent(_Data, 0, _) -> {error, retries_exceeded};
    ...
    case prompt_and_get_pin(Prompts, Data, Call) of
        {ok, Pin} -> % Pin = <<"315">>
            case couch_mgr:get_results(Db, <<"agent/listing_by_pin">>, [{<<"include_docs">>, true}, {<<"key">>, Pin}]) of
                {ok, []} -> find_agent(Data, Retries-1, Call);
                {ok, [AgentJObj]} -> {ok, AgentJObj};
                {error, _} -> find_agent(Data, Retries-1, Call)
            end;
        {error, _} ->
            cf_call_command:hangup(Call)
    end.
