...
-spec from_route_win/1 :: (wh_json:json_object()) -> call().
from_route_win(RouteWin) ->
    from_route_win(RouteWin, #whapps_call{}). 

-spec from_route_win/2 :: (wh_json:json_object(), call()) -> call().
...
-spec from_json/1 :: (wh_json:json_object()) -> call().
from_json(JObj) ->
...
-spec call_id_direct/1 :: (call()) -> 'undefined' | binary().

call_id(#whapps_call{call_id=CallId, call_id_helper=Fun}=Call) when is_function(Fun) ->
    Fun(CallId, Call);
...
call_id_helper(Fun, #whapps_call{}=Call) ->
...
-spec control_queue_direct/1 :: (call()) -> 'undefined' | binary().

control_queue(#whapps_call{control_q=ControlQ, control_q_helper=Fun}=Call) when is_function(Fun) ->
    Fun(ControlQ, Call);
...
control_queue_helper(Fun, #whapps_call{}=Call) ->
