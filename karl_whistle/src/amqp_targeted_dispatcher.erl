%%%-------------------------------------------------------------------
%%% File    : amqp_targeted_dispatcher.erl
%%% Author  : K Anderson
%%% Description : Handles rx and tx amqp of the direct exchange
%%%
%%% Created : March 24 2010
%%%-------------------------------------------------------------------
-module(amqp_targeted_dispatcher).
-vsn('1.2').
...
-define(EXCHANGE, <<"targeted_requests">>).
...
init([]) ->
    ...
    %% Declare a queue
    ?DEBUG("~p declare queue ~p~n", [self(), "targeted." ++ net_adm:localhost()]),
    Queue = list_to_binary(["targeted."|net_adm:localhost()]),
    ...
    ?DEBUG("~p bind queue ~p to exchange ~p~n", [self(), "targeted." ++ net_adm:localhost(), binary_to_list(?EXCHANGE)]),
    QueueBind = #'queue.bind'{
        ticket = Ticket,
        queue = Queue,
        exchange = ?EXCHANGE,
        routing_key = list_to_binary(lists:concat(["targeted." ++ net_adm:localhost()])),
        nowait = false,
        arguments = []
    },
    ...
    ?DEBUG("~p open consumer for ~p in ~p~n", [self(), "targeted." ++ net_adm:localhost(), binary_to_list(?EXCHANGE)]),
    ...
    ?INFO("~p amqp_targeted_dispatcher init complete!~n", [self()]),
...
handle_call(add_consumer, {From, _}, State) ->
    ?DEBUG("~p amqp_targeted_dispatcher add consumer: ~p~n", [self(), From]),
...
handle_call(remove_consumer, {From, _}, State) ->
    ?DEBUG("~p amqp_targeted_dispatcher remove consumer: ~p~n", [self(), From]),
    #state{consumers = Consumers} = State,
...
handle_call(Unhandled, _From, State) ->
    ?DEBUG("~p amqp_targeted_dispatcher unknown call: ~p~n", [self(), Unhandled]),
...
handle_cast(Unhandled, State) ->
    ?DEBUG("~p amqp_targeted_dispatcher unknown cast: ~p~n", [self(), Unhandled]),
    {noreply, State}.
...
handle_info({_, #amqp_msg{props = Props, payload = Payload}}, State) ->
    ?DEBUG("~p amqp_targeted_dispatcher recieved: ~p~n", [self(), Payload]),
    case Props#'P_basic'.content_type of
        ...
        _ContentType -> ?WARN("~p amqp_targeted_dispatcher recieved unknown msg type: ~p~n", [self(), _ContentType])
    end,
    {noreply, State};

%% catch all so we dont loose state
handle_info(Unhandled, State) ->
    ?WARN("~p amqp_targeted_dispatcher unknown request: ~p~n", [self(), Unhandled]),
    {noreply, State}.
...
publish_message(Msg, To, State) ->
    ...
    ?DEBUG("~p amqp_targeted_dispatcher publish to ~p: ~p~n", [self(), BasicPublish#'basic.publish'.routing_key, AmqpMsg#'amqp_msg'.payload]),
    amqp_channel:cast(State#state.channel, BasicPublish, AmqpMsg).

notify([], _, _) -> ok;
notify([H|T], Props, Msg) ->
    spawn(fun() -> case is_process_alive(H) of true -> H ! {targeted_msg, Props, Msg}; false -> gen_server:call(?SERVER, {remove_consumer, H}) end end),
    notify(T, Props, Msg).
