...
-type(amqp_method_field_name() ::
       ( frame_max | ticket | message_count | immediate
       | mechanisms | exclusive | consumer_tag | nowait
       | queue | consumer_count | requeue | reply_text
       | channel_id | message_count | destination | source
       | capabilities | locales | if_empty | exchange
       | heartbeat | ticket | auto_delete | auto_delete
       | consumer_tag | challenge | multiple | routing_key
       | no_ack | read | passive | exclusive
       | message_count | if_unused | exchange | method_id
       | routing_key | routing_key | nowait | delivery_tag
       | if_unused | exchange | cluster_id | internal
       | class_id | ticket | active | locale
       | nowait | method_id | routing_key | exchange
       | active | exclusive | routing_key | routing_key
       | nowait | requeue | channel_max | nowait
       | server_properties | ticket | queue | exchange
       | durable | exchange | write | arguments
       | redelivered | nowait | passive | prefetch_size
       | delivery_tag | nowait | nowait | nowait
       | known_hosts | client_properties | frame_max | redelivered
       | nowait | arguments | prefetch_count | insist
       | no_local | type | ticket | response
       | version_major | passive | arguments | ticket
       | class_id | ticket | arguments | queue
       | exchange | ticket | ticket | heartbeat
       | reply_code | routing_key | consumer_tag | requeue
       | response | source | multiple | version_minor
       | mechanism | consumer_tag | ticket | message_count
       | queue | arguments | out_of_band | ticket
       | global | mandatory | no_ack | reply_code
       | consumer_tag | queue | ticket | exchange
       | delivery_tag | channel_max | arguments | durable
       | queue | queue | virtual_host | queue
       | realm | ticket | reply_text | reply_code
       | active | routing_key | delivery_tag | delivery_tag
       | reply_text | nowait | requeue | destination
       | arguments )).
