...
-type(amqp_method_field_name() ::
       ( method_id | nowait | delivery_tag | ticket
       | consumer_tag | identifier | ticket | ticket
       | ticket | arguments | consumer_tag | reply_text
       | channel_max | heartbeat | routing_key | requeue
       | ticket | version_minor | active | delivery_tag
       | requeue | prefetch_size | reply_code | exchange
       | reply_code | delivery_tag | ticket | nowait
       | exchange | delivery_tag | arguments | meta_data
       | host | class_id | result | consumer_tag
       | queue | multiple | message_count | exchange
       | ticket | exchange | auto_delete | routing_key
       | nowait | frame_max | message_count | integer_1
       | requeue | queue | reply_text | passive
       | table | response | active | consumer_tag
       | queue | exchange | no_local | reply_code
       | consume_rate | identifier | known_hosts | consumer_tag
       | nowait | exclusive | queue | ticket
       | ticket | if_unused | internal | multiple
       | routing_key | consumer_tag | redelivered | consumer_tag
       | mechanisms | durable | nowait | class_id
       | delivery_tag | prefetch_count | exchange | exclusive
       | consumer_tag | queue | routing_key | integer_4
       | exclusive | passive | exclusive | immediate
       | cluster_id | global | integer_2 | content_checksum
       | queue | nowait | if_empty | mandatory
       | integer_op | nowait | locale | consumer_tag
       | routing_key | ticket | prefetch_size | routing_key
       | string_op | locales | method_id | no_ack
       | reply_code | queue | routing_key | queue
       | no_ack | ticket | out_of_band | realm
       | exchange | reply_code | exchange | string_1
       | queue | message_count | virtual_host | redelivered
       | durable | channel_max | mechanism | exchange
       | no_local | no_local | nowait | exchange
       | prefetch_size | content_size | write | nowait
       | dtx_identifier | queue | global | nowait
       | delivery_tag | reply_text | type | string_2
       | consumer_count | prefetch_count | reply_text | mandatory
       | consumer_tag | nowait | redelivered | consumer_tag
       | known_hosts | arguments | requeue | reply_text
       | frame_max | exchange | response | read
       | routing_key | client_properties | result | routing_key
       | nowait | insist | mandatory | arguments
       | capabilities | reply_code | queue | exchange
       | message_count | consumer_tag | delivery_tag | exclusive
       | auto_delete | integer_3 | exchange | integer_result
       | immediate | routing_key | consumer_tag | global
       | consumer_tag | prefetch_count | heartbeat | routing_key
       | operation | passive | server_properties | reply_text
       | challenge | staged_size | if_unused | ticket
       | no_ack | immediate | version_major | delivery_tag
       | ticket | ticket | identifier | details
       | string_result | ticket | active | exchange
       | operation | consumer_tag )).
