...
-spec new/0 :: () -> amqp_broker:broker().
new() ->
    #amqp_broker{}.

-spec name/1 :: (amqp_broker:broker()) -> 'undefined' | atom().
name(#amqp_broker{uri=URI}) ->
    wh_util:to_atom(URI, true).

-spec uri/1 :: (amqp_broker:broker()) -> 'undefined' | atom().
uri(#amqp_broker{uri=URI}) ->    
    URI.

-spec set_uri/2 :: (atom() | string() | ne_binary(), amqp_broker:broker()) -> amqp_broker:broker().
set_uri(URI, Broker) ->
    U = wh_util:to_list(URI),
    {ok, Params} = amqp_uri:parse(U),
    Broker#amqp_broker{uri=U, params=Params}.

-spec use_federation/1 :: (amqp_broker:broker()) -> boolean().
use_federation(#amqp_broker{use_federation=UseFederation}) ->    
    UseFederation.

-spec set_use_federation/2 :: (atom() | string() | ne_binary(), amqp_broker:broker()) -> amqp_broker:broker().
set_use_federation(UseFederation, Broker) ->
    Broker#amqp_broker{use_federation=wh_util:is_true(UseFederation)}.

-spec params/1 :: (amqp_broker:broker()) -> 'undefined' | #'amqp_params_direct'{} | #'amqp_params_network'{}.
params(#amqp_broker{params=Params}) ->    
    Params.
            
-spec is_available/1 :: (amqp_broker:broker()) -> boolean().
is_available(#amqp_broker{is_available=IsAvailable}) ->    
    IsAvailable.

-spec set_is_available/2 :: (atom() | string() | ne_binary(), amqp_broker:broker()) -> amqp_broker:broker().
