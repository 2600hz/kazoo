    ...
    case couch_mgr:open_doc(Db, Num) of
        {ok, JObj} ->
            ...
            case is_binary(AssignedTo) andalso
                ...
                true ->
                    ?LOG("number belongs to an active account ~s", [AssignedTo]),
                    {ok, AssignedTo, wh_json:is_true(<<"force_outbound">>, JObj, false)};
                false ->
                    ?LOG("number is inactive, account id is unknown, or account is disabled"),
                    {error, not_found}
            end;
...
update_numbers_on_account(Number, State, AccountId) ->
    ...
    case couch_mgr:open_doc(Db, AccountId) of
        {ok, JObj} ->
            ?LOG("setting number ~s to state ~s on the account ~s", [Number, State, AccountId]),
