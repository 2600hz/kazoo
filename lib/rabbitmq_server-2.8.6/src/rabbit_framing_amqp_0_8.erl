...
-type(amqp_method_field_name() ::
       ( prefetch_count | consumer_tag | locale | string_result
       | prefetch_count | challenge | queue | message_count
       | identifier | response | delivery_tag | consumer_tag
       | meta_data | nowait | channel_max | nowait
       | queue | heartbeat | consumer_tag | routing_key
       | channel_max | routing_key | frame_max | exchange
       | immediate | queue | heartbeat | integer_4
       | nowait | staged_size | virtual_host | routing_key
       | capabilities | insist | string_2 | prefetch_size
       | delivery_tag | known_hosts | string_op | prefetch_count
       | prefetch_size | host | known_hosts | consumer_tag
       | integer_3 | mandatory | reply_code | redelivered
       | consumer_tag | reply_text | class_id | consume_rate
       | method_id | routing_key | consumer_tag | no_local
       | no_ack | consumer_tag | delivery_tag | exchange
       | out_of_band | ticket | nowait | exchange
       | queue | active | content_checksum | consumer_tag
       | active | routing_key | no_ack | multiple
       | reply_code | reply_text | exclusive | consumer_tag
       | details | consumer_tag | queue | identifier
       | exchange | reply_code | ticket | ticket
       | reply_text | ticket | class_id | mandatory
       | method_id | consumer_tag | exchange | routing_key
       | mandatory | frame_max | global | exchange
       | delivery_tag | realm | exclusive | cluster_id
       | routing_key | reply_code | passive | active
       | write | identifier | read | delivery_tag
       | exchange | routing_key | routing_key | ticket
       | result | reply_code | integer_result | consumer_tag
       | consumer_tag | ticket | queue | ticket
       | exchange | table | type | passive
       | durable | auto_delete | internal | ticket
       | nowait | immediate | nowait | arguments
       | result | redelivered | operation | ticket
       | immediate | exchange | no_ack | server_properties
       | if_unused | exclusive | nowait | message_count
       | integer_op | integer_1 | reply_text | ticket
       | content_size | requeue | queue | delivery_tag
       | passive | global | durable | exclusive
       | multiple | auto_delete | delivery_tag | operation
       | nowait | delivery_tag | reply_code | arguments
       | requeue | queue | message_count | requeue
       | consumer_count | arguments | no_local | reply_text
       | exclusive | redelivered | reply_text | ticket
       | queue | no_local | exchange | exchange
       | routing_key | nowait | exchange | queue
       | arguments | requeue | consumer_tag | exchange
       | nowait | nowait | ticket | prefetch_size
       | ticket | queue | mechanism | nowait
       | routing_key | global | version_major | version_minor
       | integer_2 | message_count | dtx_identifier | mechanisms
       | response | locales | exchange | ticket
       | consumer_tag | client_properties | string_1 | if_unused
       | if_empty | ticket )).
