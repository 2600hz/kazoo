...
-type(amqp_method_field_name() ::
       ( nowait | routing_key | arguments | mandatory
       | exclusive | consumer_tag | queue | routing_key
       | table | exchange | active | delivery_tag
       | queue | nowait | ticket | exchange
       | identifier | prefetch_count | ticket | identifier
       | heartbeat | if_unused | consumer_tag | immediate
       | passive | nowait | ticket | if_unused
       | challenge | reply_text | routing_key | version_major
       | consumer_tag | redelivered | delivery_tag | queue
       | nowait | integer_4 | consumer_tag | passive
       | ticket | heartbeat | exchange | queue
       | arguments | prefetch_size | message_count | host
       | active | reply_code | ticket | global
       | immediate | no_ack | integer_2 | channel_max
       | exchange | nowait | no_ack | integer_op
       | response | string_result | requeue | prefetch_size
       | version_minor | prefetch_count | requeue | routing_key
       | operation | reply_code | content_size | exchange
       | method_id | active | queue | no_local
       | frame_max | reply_code | ticket | exchange
       | known_hosts | consumer_tag | exchange | virtual_host
       | method_id | multiple | queue | auto_delete
       | nowait | delivery_tag | ticket | server_properties
       | nowait | prefetch_count | mandatory | reply_text
       | locale | consumer_tag | message_count | ticket
       | reply_text | identifier | exchange | delivery_tag
       | string_2 | queue | cluster_id | content_checksum
       | integer_result | passive | nowait | no_local
       | consumer_tag | capabilities | routing_key | internal
       | auto_delete | staged_size | consumer_tag | if_empty
       | multiple | meta_data | exchange | mechanisms
       | exclusive | exchange | result | routing_key
       | queue | class_id | result | routing_key
       | ticket | client_properties | routing_key | requeue
       | reply_code | out_of_band | prefetch_size | exchange
       | delivery_tag | reply_code | routing_key | insist
       | reply_code | consumer_tag | nowait | consumer_tag
       | ticket | exclusive | nowait | redelivered
       | operation | locales | ticket | global
       | consumer_tag | no_ack | nowait | realm
       | delivery_tag | reply_text | message_count | mandatory
       | routing_key | consumer_count | delivery_tag | exchange
       | consumer_tag | reply_text | consumer_tag | message_count
       | consumer_tag | ticket | arguments | response
       | mechanism | integer_1 | integer_3 | ticket
       | write | durable | routing_key | dtx_identifier
       | queue | durable | channel_max | global
       | exclusive | delivery_tag | arguments | redelivered
       | no_local | queue | consumer_tag | exchange
       | string_op | class_id | reply_text | known_hosts
       | nowait | requeue | frame_max | exclusive
       | type | queue | details | string_1
       | consume_rate | read | ticket | exchange
       | ticket | immediate )).
