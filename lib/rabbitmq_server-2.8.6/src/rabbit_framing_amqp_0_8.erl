...
-type(amqp_method_field_name() ::
       ( consumer_tag | exchange | type | passive
       | durable | auto_delete | internal | consumer_tag
       | prefetch_count | redelivered | nowait | consumer_tag
       | exchange | arguments | string_2 | ticket
       | exchange | exchange | if_unused | string_op
       | nowait | message_count | routing_key | exchange
       | cluster_id | consumer_tag | integer_1 | ticket
       | multiple | routing_key | queue | reply_code
       | passive | string_result | reply_code | durable
       | string_1 | exclusive | auto_delete | identifier
       | nowait | ticket | ticket | ticket
       | arguments | requeue | queue | message_count
       | operation | exchange | consumer_count | staged_size
       | no_local | queue | queue | exclusive
       | no_ack | ticket | queue | exchange
       | consumer_tag | routing_key | exchange | nowait
       | exchange | routing_key | routing_key | arguments
       | exchange | nowait | consumer_tag | nowait
       | routing_key | ticket | mandatory | queue
       | queue | mandatory | nowait | ticket
       | exchange | consume_rate | message_count | mandatory
       | arguments | delivery_tag | realm | consumer_tag
       | ticket | ticket | ticket | consumer_tag
       | integer_2 | queue | queue | if_unused
       | if_empty | nowait | exclusive | delivery_tag
       | prefetch_count | exclusive | global | content_size
       | queue | message_count | response | content_checksum
       | no_ack | exclusive | integer_3 | mechanisms
       | channel_max | requeue | delivery_tag | frame_max
       | heartbeat | routing_key | consumer_tag | channel_max
       | frame_max | heartbeat | global | locales
       | table | identifier | nowait | virtual_host
       | ticket | reply_text | capabilities | delivery_tag
       | nowait | insist | integer_op | redelivered
       | reply_text | known_hosts | reply_text | prefetch_count
       | host | integer_4 | known_hosts | exchange
       | routing_key | reply_code | requeue | reply_text
       | client_properties | class_id | delivery_tag | method_id
       | result | consumer_tag | delivery_tag | no_local
       | no_ack | routing_key | routing_key | out_of_band
       | consumer_tag | nowait | active | meta_data
       | consumer_tag | delivery_tag | active | exchange
       | reply_code | response | reply_code | reply_text
       | ticket | details | result | redelivered
       | identifier | reply_code | prefetch_size | immediate
       | prefetch_size | reply_text | consumer_tag | class_id
       | method_id | exchange | routing_key | version_major
       | operation | nowait | version_minor | integer_result
       | delivery_tag | server_properties | passive | prefetch_size
       | active | write | requeue | read
       | global | challenge | mechanism | immediate
       | dtx_identifier | ticket | consumer_tag | locale
       | queue | immediate | no_local | consumer_tag
       | ticket | multiple )).
