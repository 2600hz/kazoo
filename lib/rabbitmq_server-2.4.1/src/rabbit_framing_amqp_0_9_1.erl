...
-type(amqp_method_field_name() ::
       ( reply_text | ticket | exchange | if_unused
       | queue | consumer_tag | heartbeat | reply_text
       | multiple | auto_delete | queue | exchange
       | queue | arguments | durable | channel_id
       | active | class_id | nowait | delivery_tag
       | source | ticket | immediate | cluster_id
       | if_unused | ticket | internal | destination
       | locales | ticket | destination | passive
       | routing_key | class_id | active | if_empty
       | exchange | ticket | routing_key | exchange
       | source | exchange | server_properties | insist
       | challenge | routing_key | nowait | nowait
       | method_id | nowait | redelivered | consumer_count
       | no_ack | delivery_tag | auto_delete | active
       | message_count | nowait | routing_key | routing_key
       | exchange | nowait | frame_max | global
       | arguments | frame_max | mechanisms | nowait
       | nowait | ticket | exchange | message_count
       | client_properties | exclusive | routing_key | write
       | nowait | locale | version_major | prefetch_size
       | nowait | exclusive | ticket | known_hosts
       | ticket | mandatory | consumer_tag | heartbeat
       | requeue | routing_key | ticket | multiple
       | type | read | reply_text | arguments
       | arguments | ticket | passive | prefetch_count
       | no_ack | arguments | consumer_tag | queue
       | exchange | no_local | delivery_tag | method_id
       | ticket | redelivered | channel_max | response
       | ticket | queue | version_minor | requeue
       | queue | consumer_tag | reply_code | routing_key
       | queue | reply_code | delivery_tag | out_of_band
       | mechanism | requeue | reply_code | arguments
       | virtual_host | message_count | ticket | response
       | message_count | durable | exclusive | consumer_tag
       | nowait | queue | capabilities | requeue
       | realm | passive | channel_max | delivery_tag
       | arguments )).
