...
-type(amqp_method_field_name() ::
       ( ticket | channel_max | queue | routing_key
       | result | exchange | routing_key | consumer_tag
       | read | prefetch_size | channel_max | known_hosts
       | reply_code | exchange | exchange | insist
       | delivery_tag | exclusive | auto_delete | no_local
       | consume_rate | immediate | nowait | message_count
       | operation | message_count | redelivered | requeue
       | reply_text | routing_key | operation | active
       | global | staged_size | no_local | routing_key
       | ticket | nowait | ticket | ticket
       | passive | ticket | ticket | exchange
       | if_unused | consumer_tag | internal | capabilities
       | consumer_tag | consumer_tag | ticket | table
       | mechanisms | ticket | routing_key | frame_max
       | class_id | consumer_tag | reply_text | class_id
       | exchange | queue | requeue | nowait
       | queue | global | heartbeat | delivery_tag
       | consumer_tag | reply_code | reply_code | nowait
       | locales | delivery_tag | integer_3 | nowait
       | string_op | challenge | virtual_host | exclusive
       | active | string_2 | multiple | queue
       | meta_data | method_id | arguments | exchange
       | reply_code | routing_key | arguments | nowait
       | message_count | host | mandatory | routing_key
       | requeue | queue | reply_text | queue
       | consumer_tag | delivery_tag | queue | message_count
       | queue | exchange | result | exchange
       | arguments | nowait | identifier | identifier
       | exchange | ticket | write | routing_key
       | integer_4 | ticket | consumer_tag | queue
       | exchange | reply_text | nowait | locale
       | routing_key | redelivered | consumer_count | delivery_tag
       | consumer_tag | string_1 | nowait | auto_delete
       | passive | no_ack | known_hosts | out_of_band
       | consumer_tag | immediate | routing_key | frame_max
       | exclusive | type | server_properties | global
       | immediate | client_properties | integer_2 | content_checksum
       | no_ack | nowait | exclusive | integer_op
       | prefetch_size | details | consumer_tag | routing_key
       | delivery_tag | if_empty | content_size | ticket
       | exchange | method_id | prefetch_size | delivery_tag
       | reply_code | queue | prefetch_count | ticket
       | ticket | heartbeat | no_ack | integer_result
       | integer_1 | consumer_tag | requeue | passive
       | exchange | mandatory | multiple | mechanism
       | exclusive | version_major | consumer_tag | redelivered
       | identifier | queue | nowait | dtx_identifier
       | active | prefetch_count | mandatory | nowait
       | if_unused | arguments | delivery_tag | string_result
       | ticket | consumer_tag | cluster_id | version_minor
       | ticket | prefetch_count | no_local | reply_text
       | durable | response | response | consumer_tag
       | realm | durable | reply_text | reply_code
       | exchange | exchange )).
