...
%% API
-export([start_link/0, start_link/1, store/2, store/3, peak/1, fetch/1, erase/1, flush/0, fetch_keys/0, filter/1]).
-export([start_local_link/0, store_local/3, store_local/4, peak_local/2, fetch_local/2,  erase_local/2
...
-spec(peak/1 :: (K :: term()) -> tuple(ok, term()) | tuple(error, not_found)).
peak(K) ->
    gen_server:call(?SERVER, {peak, K}).
...
-spec(peak_local/2 :: (Srv :: pid(), K :: term()) -> tuple(ok, term()) | tuple(error, not_found)).
peak_local(Srv, K) when is_pid(Srv) ->
    gen_server:call(Srv, {peak, K}).
