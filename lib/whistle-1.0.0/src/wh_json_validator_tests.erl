...
card_test() ->
    ...
    Failure = wh_json:to_proplist(
                wh_json:decode(
                  <<"{ \"givenName\": { \"required\": \"Field is required but missing\" }, \"familyName\": { \"required\": \"Field is required but missing\" }}">>
                 )),
    CardJObj = wh_json:new(),
    {fail, Bad} = wh_json_validator:is_valid(CardJObj, CardSchema),

    assert_equal(Failure, Bad).

assert_equal([], []) -> ok;
assert_equal([{KFail,_}|Fail], [{[KBad],_}|Bad]) ->
    ?assertEqual(KFail, KBad),
    assert_equal(Fail, Bad);
assert_equal([{KFail,_}|Fail], [{KBad,_}|Bad]) ->
    ?assertEqual(KFail, KBad),
    assert_equal(Fail, Bad).
...
address_test() ->
    ...
    Failure = wh_json:to_proplist(
                wh_json:decode(
                  <<"{ \"country-name\": { \"required\": \"Field is required but missing\" }, \"region\": { \"required\": \"Field is required but missing\" }, \"locality\": { \"required\": \"Field is required but missing\" }}">>
                 )),
    AddressJObj = wh_json:new(),

    {fail, Bad} = wh_json_validator:is_valid(AddressJObj, AddressSchema),
    assert_equal(Failure, Bad).
