...
-define(LOG_START(Format), 
        logger:info("~s|start|~p:~b (~w) " ++ Format, [get(callid), ?MODULE, ?LINE, self()])).
-define(LOG_START(Format, Data), 
        logger:info("~s|start|~p:~b (~w) " ++ Format, [get(callid), ?MODULE, ?LINE, self()] ++ Data)).
-define(LOG_START(CallId, Format, Data), 
        logger:info("~s|start|~p:~b (~w) " ++ Format, [CallId, ?MODULE, ?LINE, self()] ++ Data)).


-define(LOG(Format), 
        logger:info("~s|log|~p:~b (~w) " ++ Format, [get(callid), ?MODULE, ?LINE, self()])).
-define(LOG(Format, Data), 
        logger:info("~s|log|~p:~b (~w) " ++ Format, [get(callid), ?MODULE, ?LINE, self()] ++ Data)).
-define(LOG(CallId, Format, Data), 
        logger:info("~s|log|~p:~b (~w) " ++ Format, [CallId, ?MODULE, ?LINE, self()] ++ Data)).


-define(LOG_END(Format), 
        logger:info("~s|end|~p:~b (~w) " ++ Format, [get(callid), ?MODULE, ?LINE, self()])).
-define(LOG_END(Format, Data), 
        logger:info("~s|end|~p:~b (~w) " ++ Format, [get(callid), ?MODULE, ?LINE, self()] ++ Data)).
