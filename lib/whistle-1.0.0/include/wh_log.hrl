...
-define(LOG_START(Format__For__Log__),
        (fun(DebugMsg__) ->
                 ...
                 logger:debug("~s|start|~p:~b (~w) " ++ DebugMsg__, [LogCallId, ?MODULE, ?LINE, self()])
...
-define(LOG_START(Format__For__Log__, Data__For__Format__),
        (fun(LogLevel__, DebugMsg__) when is_atom(LogLevel__) ->
                 LogCallId = ?LOG_CALLID_FUN(erlang:get(callid)),
                 logger:LogLevel__("~s|start|~p:~b (~w) " ++ DebugMsg__, [LogCallId, ?MODULE, ?LINE, self()]);
             (DebugMsg__, DebugData__) ->
                 LogCallId = ?LOG_CALLID_FUN(erlang:get(callid)),
                 logger:debug("~s|start|~p:~b (~w) " ++ DebugMsg__, [LogCallId, ?MODULE, ?LINE, self() | DebugData__])
...
-define(LOG_START(CallId__, Format__For__Log__, Data__For__Format__),
        (fun(LogLevel__, DebugMsg__, DebugData__) when is_atom(LogLevel__) ->
                 LogCallId = ?LOG_CALLID_FUN(erlang:get(callid)),
                 logger:LogLevel__("~s|start|~p:~b (~w) " ++ DebugMsg__, [LogCallId, ?MODULE, ?LINE, self() | DebugData__]);
            (DebugCallID__, DebugMsg__, DebugData__) ->
                 logger:debug("~s|start|~p:~b (~w) " ++ DebugMsg__, [DebugCallID__, ?MODULE, ?LINE, self() | DebugData__])
...
-define(LOG_START(LogLevel__, CallId__, Format__For__Log__, Data__For__Format__),
        (fun(DebugLogLevel__, DebugCallID__, DebugMsg__, DebugData__) when is_atom(DebugLogLevel__) ->
                 logger:DebugLogLevel__("~s|start|~p:~b (~w) " ++ DebugMsg__, [LogCallId, ?MODULE, ?LINE, self() | DebugData__]);
...
-define(LOG_STACKTRACE(StacktraceData__),
        (fun(DebugST__) ->
                 LogCallId = ?LOG_CALLID_FUN(erlang:get(callid)),
                 [logger:debug("~s|start|~p:~b (~w) st: ~p", [LogCallId, ?MODULE, ?LINE, self(), STLine__]) || STLine__ <- DebugST__]
