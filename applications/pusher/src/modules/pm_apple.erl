...
-include("../lib/apns/include/apns.hrl").
-include("../lib/apns/include/localized.hrl").
...
-spec get_apns(api_binary(), ets:tid()) -> pid() | 'undefined'.
...
-spec maybe_load_apns(api_binary(), ets:tid()) -> pid() | 'undefined'.
maybe_load_apns(App, ETS) ->
    maybe_load_apns(App, ETS, whapps_config:get_binary(?CONFIG_CAT, <<"apple">>, 'undefined', App)).

-spec maybe_load_apns(api_binary(), ets:tid(), api_binary()) -> pid() | 'undefined'.
...
maybe_load_apns(App, ETS, CertBin) ->
    ...
    case apns:connect(#apns_connection{cert=Cert, key=Key}) of
        ...
        {'error', {already_started, Pid}} -> ets:insert(ETS, {App, Pid}),
                                             Pid; 
        {'error', Reason} -> lager:error("Error loading apns ~p", [Reason]),
                             'undefined'
    end.
