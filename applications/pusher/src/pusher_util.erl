...
-type keycert() :: {maybe({'PrivateKeyInfo', binary()})
                    ,maybe(binary())
                   }.
...
-spec user_agent_push_properties(ne_binary()) -> maybe(kz_json:object()).
-spec user_agent_push_properties(ne_binary(), kz_json:objects()) -> maybe(kz_json:object()).
