...
%%% @contributors
%%%-------------------------------------------------------------------
-module(notify_password_recovery_req).

-export([init/0, handle_req/2]).

-include("notify.hrl").

-define(DEFAULT_TEXT_TMPL, 'notify_password_recovery_req_text_tmpl').
-define(DEFAULT_HTML_TMPL, 'notify_password_recovery_req_html_tmpl').
-define(DEFAULT_SUBJ_TMPL, 'notify_password_recovery_req_subj_tmpl').

-define(MOD_CONFIG_CAT, <<(?NOTIFY_CONFIG_CAT)/binary, ".password_recovery_req">>).
...
handle_req(JObj, _Props) ->
    'true' = wapi_notifications:pwd_recovery_req_v(JObj),
    wh_util:put_callid(JObj),

    lager:debug("request for password request taken into account, sending email notification"),
    ...
    CustomTxtTemplate = wh_json:get_value([<<"notifications">>, <<"password_recovery_req">>, <<"email_text_template">>], Account),
    {'ok', TxtBody} = notify_util:render_template(CustomTxtTemplate, ?DEFAULT_TEXT_TMPL, Props),

    CustomHtmlTemplate = wh_json:get_value([<<"notifications">>, <<"password_recovery_req">>, <<"email_html_template">>], Account),
    {'ok', HTMLBody} = notify_util:render_template(CustomHtmlTemplate, ?DEFAULT_HTML_TMPL, Props),

    CustomSubjectTemplate = wh_json:get_value([<<"notifications">>, <<"password_recovery_req">>, <<"email_subject_template">>], Account),
