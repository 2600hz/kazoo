...
%%% API
%%%===================================================================
-spec get_cdr_doc_id(pos_integer(), api(binary())) -> ne_binary().
get_cdr_doc_id(Timestamp, CallId) ->
    {{Year, Month, _}, _} = calendar:gregorian_seconds_to_datetime(Timestamp),
    get_cdr_doc_id(Year, Month, CallId).

-spec get_cdr_doc_id(pos_integer(), pos_integer(), api(binary())) -> ne_binary().
get_cdr_doc_id(Year, Month, CallId) ->
...
-spec save_cdr(api(binary()), kz_json:object()) ->
                      'ok' | kz_std_return().
...
-spec save_cdr(api(binary()), kz_json:object(), 0..?MAX_RETRIES) ->
                      {'error', any()} | 'ok'.
