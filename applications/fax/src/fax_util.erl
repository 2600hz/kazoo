...
save_fax_attachment(JObj, FileContents, CT, Name, Count) ->
    ...
    case check_fax_attachment(DocId, Name) of
        ...
        {'missing', J} ->
            lager:warning("missing fax attachment ~s on fax id ~s rev ~s",[DocId, Name, kz_doc:revision(JObj)]),
        ...
        {'error', _R} ->
            lager:debug("error '~p' saving fax attachment ~s on fax id ~s rev ~s"
                       ,[_R, Name, DocId, kz_doc:revision(JObj)]
                       ),
            timer:sleep(?RETRY_SAVE_ATTACHMENT_DELAY),
