...
-spec init(kz_json:object(), maybe(binary()) | maybe([maybe(binary())])) ->
                  state() |
...
-spec get_custom_sip_headers(state()) -> maybe(kz_json:object()).
...
-spec get_aleg_id(state()) -> maybe(binary()).
-spec get_bleg_id(state()) -> maybe(binary()).
...
-spec get_failover(state()) -> maybe(kz_json:object()).
get_failover(#ts_callflow_state{failover=Fail}) -> Fail.

-spec is_trunkstore_acct(kz_json:object(), maybe(binary()) | maybe([maybe(binary())])) -> boolean().
