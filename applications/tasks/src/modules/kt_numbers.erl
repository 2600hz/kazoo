...
list_number(N) ->
    ...
    #{<<"e164">> => knm_phone_number:number(PN)
     ...
     ,<<"ported_in">> => kz_util:to_binary(knm_phone_number:ported_in(PN))
     ,<<"carrier_module">> => knm_phone_number:module_name(PN)
     ,<<"cnam.inbound">> => kz_util:to_binary(kz_json:is_true(?CNAM_INBOUND_LOOKUP, InboundCNAM))
     ...
     ,<<"prepend.enabled">> => kz_util:to_binary(kz_json:is_true(?PREPEND_ENABLED, Prepend))
     ...
     ,<<"force_outbound">> => kz_util:to_binary(knm_number:force_outbound_feature(PN))
...
handle_result(Args, {error, Reason})
  when is_atom(Reason) ->
    format_result(Args, kz_util:to_binary(Reason));
