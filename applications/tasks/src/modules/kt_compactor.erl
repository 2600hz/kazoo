...
-spec ensure_dbs_view() -> 'ok'.
ensure_dbs_view() ->
    kz_datamgr:revise_doc_from_file(kazoo_couch:get_admin_dbs()
                                   ,'tasks'
                                   ,filename:join(["views", "compactor.json"])
                                   ),
    'ok'.
...
-spec compact_db(ne_binary()) -> rows().
compact_db(Database) ->
    do_compact_db(Database, ?HEUR_NONE).

-spec heuristic_from_flag(api_ne_binary()) -> heuristic().
...
-spec compact_node(ne_binary()) -> rows().
compact_node(Node) ->
    do_compact_node(Node, ?HEUR_NONE).

-spec do_compact_node(ne_binary(), heuristic()) ->
                             rows().
...
-spec do_compact_node(ne_binary(), heuristic(), kz_data:connection(), kz_data:connection()) ->
                             rows().
do_compact_node(Node, Heuristic, APIConn, AdminConn) ->
...
-spec do_compact_node(ne_binary(), heuristic(), kz_data:connection(), kz_data:connection(), ne_binaries()) -> rows().
...
-spec do_compact_node_db(ne_binary(), heuristic(), kz_data:connection(), kz_data:connection(), ne_binary(), rows()) -> rows().
do_compact_node_db(Node, Heuristic, APIConn, AdminConn, Database, Acc) ->
...
-spec do_compact_db(ne_binary()) -> rows().
-spec do_compact_db(ne_binary(), heuristic()) -> rows().
...
-spec do_compact_db_fold(ne_binary(), rows()) -> rows().
do_compact_db_fold(Database, Rows) ->
    Rows ++ do_compact_db(Database).

-spec do_compact_db_by_nodes(ne_binary(), heuristic()) -> rows().
do_compact_db_by_nodes(Database, Heuristic) ->
...
-spec do_compact_db_by_node(ne_binary(), heuristic(), ne_binary(), rows()) -> rows().
...
-spec do_compact_db_by_node(ne_binary(), heuristic(), kz_data:connection(), kz_data:connection(), ne_binary(), rows()) -> rows().
do_compact_db_by_node(Node, Heuristic, APIConn, AdminConn, Database, Acc) ->
...
-spec db_usage_cols(kz_data:connection(), ne_binary()) -> [integer()].
...
-spec node_compactor(ne_binary(), heuristic(), kz_data:connection(), kz_data:connection(), ne_binary()) ->
...
-spec get_node_connections(ne_binary(), kz_data:connection()) ->
