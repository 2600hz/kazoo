...
do_lookup(Number, AccountId) ->
    ...
    Db = kzs_util:format_account_db(AccountId),
    Options = [{'keys', [[kzd_callflows:type(), <<"by_number">>, Number]
                        ,[<<"textflow">>, <<"by_number">>, Number]
                        ]
               }
              ,'include_docs'
              ],
...
get_first_flow([JObj | _]=JObjs) ->
    Fun = fun(J) ->
     ...
     case lists:partition(Fun, JObjs) of
         {[], [JObj | _]} -> JObj;
         {[JObj | _], _} -> JObj
     end.
...
load_patterns(AccountId) ->
    Options = [{'key', [<<"textflow">>, <<"by_number">>, Number]}
              ,'include_docs'
              ],
    case kz_datamgr:get_results(Db, ?KZ_VIEW_LIST_UNIFORM, Options) of
        {'ok', []} -> {'error', 'not_found'};
