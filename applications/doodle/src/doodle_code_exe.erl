...
handle(Metaflow, Call) ->
    ...
    try (wh_util:to_atom(<<"df_", M/binary>>)):handle(Data, Call) of
        {'branch', ChildBranch, Call1} ->
            lager:debug("continuing to child callflow from doodle_~s", [M]),
        ...
        _ -> lager:debug("finished handling callflow for df_~s", [M])
    catch
        _E:_R ->
