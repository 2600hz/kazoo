...
handle_cast({'job_complete', Worker}, #state{jobs=Jobs}=State) ->
    poolboy:checkin('doodle_worker_pool', Worker),
...
handle_info('expire_jobs', State) ->
    ...
    case couch_mgr:get_results(?DOODLE_DB, <<"sms/processing_by_node">>, ViewOptions) of
        {'ok', JObjs} ->
            [begin
                 DocId = wh_json:get_value(<<"id">>, JObj),
                 lager:debug("moving expired job ~s status to pending", [DocId]),
                 couch_mgr:update_doc(?DOODLE_DB, DocId, [{<<"pvt_job_status">>, <<"pending">>}])
             end
             || JObj <- JObjs
...
handle_info('timeout', #state{jobs=[]}=State) ->
    ...
    case couch_mgr:get_results(?DOODLE_DB, <<"sms/pending">>, ViewOptions) of
        {'ok', []} -> {'noreply', State, ?POLLING_INTERVAL};
        {'ok', Jobs} ->
            lager:debug("fetched ~b jobs, attempting to distribute to workers", [length(Jobs)]),
            {'noreply', State#state{jobs=distribute_jobs(Jobs)}, ?POLLING_INTERVAL};
        {'error', _Reason} ->
            lager:debug("failed to fetch sms jobs: ~p", [_Reason]),
            {'noreply', State, ?POLLING_INTERVAL}
    end;
...
terminate(_Reason, _State) ->
    lager:debug("doodle jobs terminating: ~p", [_Reason]).
...
distribute_jobs([Job|Jobs]) ->
    case catch poolboy:checkout('doodle_worker_pool', 'false', 1000) of
