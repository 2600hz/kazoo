...
-record(state, {voice_uri :: maybe(binary())
                ,cdr_uri :: maybe(binary())
                ,request_format = <<"twiml">> :: maybe(binary())
...
-spec handle_resp(maybe(binary()), kapps_call:call(), ne_binary(), binary()) -> 'ok'.
...
-spec process_resp(maybe(binary()), kapps_call:call(), list() | binary(), binary()) ->
                          {'stop', kapps_call:call()} |
                          {'ok', kapps_call:call()} |
