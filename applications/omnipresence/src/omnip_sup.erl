...
start_module(Module) ->
    supervisor:start_child(?SERVER, ?WORKER(kz_term:to_atom(Module, 'true'))).

-spec stop_module(ne_binary()) -> 'ok' | {'error', any()}.
stop_module(Module) ->
    case supervisor:terminate_child(?SERVER, kz_term:to_atom(Module, 'true')) of
        'ok' ->
            _ = supervisor:delete_child(?SERVER, kz_term:to_atom(Module, 'true')),
            'ok';
        {'error', _}=E -> E
    end.
