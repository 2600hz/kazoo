{application,crossbar,
             [{description,"Crossbar - REST Interface to the stars"},
              {vsn,"0.3.5"},
              {modules,[accounts,callflows,conferences,crossbar,crossbar_app,
                        crossbar_bindings,crossbar_doc,crossbar_module_sup,
                        crossbar_resource,crossbar_session,crossbar_sup,
                        crossbar_util,crossbar_validator,devices,evtsub,media,
                        noauthn,noauthz,nosipauth,plists,static_resource,
                        t_evtsub,users,v1_resource,vmboxes]},
              {registered,[]},
              {applications,[kernel,stdlib,crypto,mochiweb,webmachine]},
              {mod,{crossbar_app,[]}},
              {env,[]}]}.
