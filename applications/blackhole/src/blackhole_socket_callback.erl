...
%% @end
%%--------------------------------------------------------------------
-spec recv(ne_binary(), ne_binary(), any(), bh_context:context()) -> cb_return() | 'ok'.
...
%% @end
%%--------------------------------------------------------------------
-spec subscribe(bh_context:context(), wh_json:object()) -> cb_return() | 'ok'.
-spec subscribe(bh_context:context(), wh_json:object(), boolean()) -> cb_return() | 'ok'.
-spec subscribe(bh_context:context(), wh_json:object(), ne_binary(), api_binary()) -> cb_return() | 'ok'.
...
subscribe(Context, _JObj, 'false') ->
    blackhole_util:respond_with_authn_failure(Context);
...
subscribe(Context, _JObj, _Binding, 'undefined') ->
    blackhole_util:respond_with_error(Context);
...
%% @end
%%--------------------------------------------------------------------
-spec unsubscribe(bh_context:context(), wh_json:object(), ne_binary(), ne_binary()) -> cb_return() | 'ok'.
-spec unsubscribe(bh_context:context(), wh_json:object(), ne_binary(), ne_binary(), boolean()) -> cb_return() | 'ok'.
...
%% @end
%%--------------------------------------------------------------------
-spec unsubscribe_for_all(bh_context:context(), wh_json:object(), ne_binary(), ne_binary()) -> cb_return() | 'ok'.
unsubscribe_for_all(Context, JObj, SessionPid, SessionId) ->
...
%% @end
%%--------------------------------------------------------------------
-spec unsubscribe_for_account(bh_context:context(), wh_json:object(), ne_binary()) -> cb_return() | 'ok'.
-spec unsubscribe_for_account(bh_context:context(), wh_json:object(), ne_binary(), ne_binary(), api_binary()) -> cb_return() | 'ok'.
unsubscribe_for_account(Context, JObj, AccountId) ->
    Binding = wh_json:get_value(<<"binding">>, JObj),
    Module = blackhole_util:get_callback_module(Binding),
