...
request(#{control_q := ControlQ, control_p := ControlP, payload := JObj}=Map) ->
    Map#{request => kz_json:set_value(?KEY_SERVER_ID, kapi:encode_pid(ControlQ, ControlP), JObj)}.
...
timeout(#{payload := JObj}=Map) ->
    T0 = kz_json:get_integer_value(<<"Fetch-Timestamp-Micro">>, JObj, erlang:system_time('micro_seconds')),
    T1 = kz_json:get_integer_value(<<"Fetch-Timeout">>, JObj, 3500000),
...
maybe_expired(#{timeout := Timeout}=Map)
  when Timeout =< 0 ->
    maybe_expired(Map#{timeout := 5000});
