...
-record(sip_subscription, {key :: maybe(binary()) | '_'
                           ,to :: maybe(binary()) | '$1' | '_'
                           ,from :: maybe(binary()) | '$2' | '_'
                           ,node :: atom() | '$1' | '_'
                           ,expires = 300 :: pos_integer() | '$1' | '_'
                           ,timestamp = kz_util:current_tstamp() :: pos_integer() | '$2' | '_'
                          }).

-record(channel, {uuid :: maybe(binary()) | '$1' | '_'
                  ,destination :: maybe(binary()) | '_'
                  ,direction :: maybe(binary()) | '$1' | '_'
                  ,account_id :: maybe(binary()) | '$1' | '$2' | '_'
                  ,account_billing :: maybe(binary()) | '$7' | '_'
                  ,authorizing_id :: maybe(binary()) | '$1' | '$3' | '_'
                  ,authorizing_type :: maybe(binary()) | '_'
                  ,owner_id :: maybe(binary()) | '$1' | '_'
                  ,resource_id :: maybe(binary()) | '$4' | '_'
                  ,presence_id :: maybe(binary()) | '$2' | '_'
                  ,fetch_id :: maybe(binary()) | '$5' | '_'
                  ,bridge_id :: maybe(binary()) | '$5' | '_'
                  ,reseller_id :: maybe(binary()) | '_'
                  ,reseller_billing :: maybe(binary()) | '_'
                  ,realm :: maybe(binary()) | '_' | '$2'
                  ,username :: maybe(binary()) | '_' | '$1'
                  ,import_moh = 'false' :: boolean() | '_'
                  ,answered = 'true' :: boolean() | '_'
                  ,other_leg :: maybe(binary()) | '$2' | '_'
                  ,node :: atom() | '$1' | '$2' | '$3' | '_'
                  ,former_node :: atom() | '$2' | '_'
                  ,timestamp :: gregorian_seconds() | '$3' | '_'
                  ,profile :: maybe(binary()) | '_'
                  ,context :: maybe(binary()) | '_'
                  ,dialplan :: maybe(binary()) | '_'
                  ,precedence = 5 :: pos_integer() | '$2' | '_'
                  ,handling_locally = 'false' :: boolean() | '_' %% is this ecallmgr handling the call control?
                  ,to_tag :: maybe(binary()) | '_'
                  ,from_tag :: maybe(binary()) | '_'
                  ,interaction_id :: maybe(binary()) | '$5' | '_'
                  ,callee_number :: maybe(binary()) | '$5' | '_'
                  ,callee_name :: maybe(binary()) | '$5' | '_'
                 }).

-type channel() :: #channel{}.
-type channels() :: [channel()].

-record(conference, {name :: maybe(binary()) | '$1' | '_'
                     ,uuid :: maybe(binary()) | '$1' | '_'
                     ,node :: atom() | '$1' | '$2' | '_'
                     ,participants = 0 :: non_neg_integer() | '_'
                     ,profile_name = <<"default">> :: ne_binary() | '_'
                     ,with_floor :: maybe(non_neg_integer()) | '_' % which participant has the floor
                     ,lost_floor :: maybe(non_neg_integer()) | '_' % which participant has lost the floor
                     ...
                     ,switch_hostname :: maybe(binary()) | '_'
                     ,switch_url :: maybe(binary()) | '_'
                     ,switch_external_ip :: maybe(binary()) | '_'
...
-record(participant, {uuid :: maybe(binary()) | '$1' | '_'
                      ,node :: atom() | '$2' | '_'
                      ,conference_uuid :: maybe(binary()) | '$1'| '_'
                      ,conference_name :: maybe(binary()) | '$1'| '_'
