...
-type authz_reply() :: boolean() | {'true', wh_json:object()}.

-spec authorize(wh_proplist(), ne_binary(), atom()) -> authz_reply().
...
-spec maybe_authorize_channel(wh_proplist(), atom()) -> authz_reply().
    ...
    case kzd_freeswitch:channel_authorized(Props) of
        ...
        _Else ->
            case kzd_freeswitch:hunt_destination_number(Props) of
                <<"conference">> -> 'true';
                _Hunt -> maybe_channel_recovering(Props, CallId, Node)
            end
    end.

-spec maybe_channel_recovering(wh_proplist(), ne_binary(), atom()) -> authz_reply().
...
-spec is_authz_enabled(wh_proplist(), ne_binary(), atom()) -> authz_reply().
...
-spec is_global_resource(wh_proplist(), ne_binary(), atom()) -> authz_reply().
...
-spec is_consuming_resource(wh_proplist(), ne_binary(), atom()) -> authz_reply().
...
-spec is_consuming_outbound_resource(wh_proplist(), ne_binary(), atom()) -> authz_reply().
is_consuming_outbound_resource(Props, CallId, Node) ->
...
-spec is_consuming_inbound_resource(wh_proplist(), ne_binary(), atom()) -> authz_reply().
is_consuming_inbound_resource(Props, CallId, Node) ->
...
-spec authz_response(wh_json:object(), wh_proplist(), ne_binary(), atom()) -> authz_reply().
authz_response(JObj, Props, CallId, Node) ->
    'ok' = set_ccv_trunk_usage(JObj, Props, CallId, Node),
...
-spec set_ccv_trunk_usage(wh_json:object(), wh_proplist(), ne_binary(), atom()) ->
                                 authz_reply().
...
-spec rate_call(wh_proplist(), ne_binary(), atom()) -> authz_reply().
rate_call(Props, CallId, Node) ->
    _P = wh_util:spawn(?MODULE, 'rate_channel', [Props, Node]),
    lager:debug("rating call in ~p", [_P]),
    allow_call(Props, CallId, Node).

-spec allow_call(wh_proplist(), ne_binary(), atom()) -> authz_reply().
allow_call(Props, _CallId, _Node) ->
    ...
    case props:is_true(<<"Call-Setup">>, Props, 'false') of
        'false' -> {'true', wh_json:from_list(Vars)};
        'true' -> 'true'
    end.
