    ...
    case kz_datamgr:open_doc(AccountDb, ?DB_DOC_NAME) of
        {'error', 'not_found'} ->
            Generators = [fun(J) -> kz_doc:set_id(J, <<"parked_calls">>) end
                         ...
                         ,fun(J) -> kz_doc:set_created(J, kz_time:current_tstamp()) end
                         ,fun(J) -> kz_doc:set_modified(J, kz_time:current_tstamp()) end
...
update_presence(State, Slot) ->
    ...
    ToUser = kz_json:get_value(<<"CID-Name">>, Slot),
    To = kz_json:get_value(<<"CID-URI">>, Slot),
