...
maybe_start_account_recording(FromNetwork, ToNetwork, Call) ->
    ...
    case maybe_start_call_recording(?ACCOUNT_INBOUND_RECORDING(FromNetwork)
                                   ,?ACCOUNT_INBOUND_RECORDING_LABEL(FromNetwork)
                                   ,Endpoint
                                   ,Call
                                   )
    of
        {'true', NewCall} -> kapps_call:set_is_recording('true', NewCall);
        'false' ->
            case maybe_start_call_recording(?ACCOUNT_OUTBOUND_RECORDING(ToNetwork)
                                           ,?ACCOUNT_OUTBOUND_RECORDING_LABEL(ToNetwork)
                                           ,Endpoint
                                           ,Call
                                           )
            of
...
maybe_start_onnet_endpoint_recording(EndpointId, ToNetwork, 'false', Call) ->
    case kz_endpoint:get(EndpointId, Call) of
        {'ok', Endpoint} ->
            maybe_start_call_recording(?ENDPOINT_OUTBOUND_RECORDING(ToNetwork)
                                      ,?ENDPOINT_OUTBOUND_RECORDING_LABEL(ToNetwork)
                                      ,Endpoint
                                      ,Call
                                      );
        _ -> 'false'
...
maybe_start_call_forwarded_recording(EndpointId, Call, {'ok', Endpoint}) ->
    InceptionNetwork = kapps_call:custom_channel_var(<<"Call-Forward-From">>, Call),

    Data = kz_json:get_json_value(?ENDPOINT_INBOUND_RECORDING(InceptionNetwork), Endpoint),
    ...
    of
        'false' -> 'false';
        'true' ->
            App = kz_endpoint_recording:record_call_command(kz_doc:id(Endpoint), InceptionNetwork, Data, Call),
            NewActions = kz_json:set_value([<<"Execute-On-Answer">>, <<"Record-Endpoint">>], App, kz_json:new()),
            lager:info("setting endpoint ~s to record on answer", [EndpointId]),
            {'true', kapps_call:kvs_store('outbound_actions', NewActions, Call)}
    end.

-spec maybe_start_call_recording(kz_term:ne_binaries(), kz_term:ne_binary(), kz_json:object(), kapps_call:call()) ->
                                        {'true', kapps_call:call()} | 'false'.
...
-spec maybe_start_call_recording(kz_term:api_object(), kz_term:ne_binary(), kapps_call:call()) ->
                                        {'true', kapps_call:call()} | 'false'.
maybe_start_call_recording('undefined', _Label, _Call) -> 'false';
maybe_start_call_recording(Data, Label, Call) ->
    case kz_json:is_true(<<"enabled">>, Data) of
        'false' -> 'false';
        'true' ->
            lager:info("starting call recording by configuration: ~s", [Label]),
            Call1 = kapps_call:kvs_store('recording_follow_transfer', 'false', Call),
            {'true', kapps_call:start_recording(kz_json:set_value(<<"origin">>, Label, Data), Call1)}
    end.
