...
-spec maybe_vm_mwi_resp(api(binary()), ne_binary(), ne_binary(), kz_json:object()) -> 'ok'.
...
-type mwi_update_return() :: 'missing_account_db' |
                             'missing_owner_id'.
-spec unsolicited_owner_mwi_update(api(binary()), api(binary())) ->
                                          'ok' |
...
-spec unsolicited_endpoint_mwi_update(api(binary()), api(binary())) ->
                                             'ok' | {'error', any()}.
...
%% @end
%%--------------------------------------------------------------------
-spec ignore_early_media(kz_json:objects()) -> api(binary()).
...
%% @end
%%--------------------------------------------------------------------
-spec correct_media_path(api(binary()), kapps_call:call()) -> api(binary()).
...
%% @end
%%--------------------------------------------------------------------
-spec handle_bridge_failure({'fail', kz_json:object()} | api(binary()), kapps_call:call()) ->
                                   'ok' | 'not_found'.
...
%% @end
%%--------------------------------------------------------------------
-spec get_sip_realm(kz_json:object(), ne_binary()) -> api(binary()).
...
-spec get_account_realm(ne_binary(), api(binary())) -> api(binary()).
...
%% @end
%%--------------------------------------------------------------------
-spec get_endpoint_owner(kz_json:object()) -> api(binary()).
get_endpoint_owner(JObj) ->
    maybe_get_endpoint_hotdesk_owner(JObj).

-spec maybe_get_endpoint_hotdesk_owner(kz_json:object()) -> api(binary()).
maybe_get_endpoint_hotdesk_owner(JObj) ->
...
-spec maybe_get_endpoint_assigned_owner(kz_json:object()) -> api(binary()).
maybe_get_endpoint_assigned_owner(JObj) ->
...
-spec maybe_start_metaflows(kapps_call:call(), kz_json:object(), api(binary())) -> 'ok'.
...
-spec sip_user_from_device_id(ne_binary(), kapps_call:call()) -> api(binary()).
...
%% @end
%%--------------------------------------------------------------------
-spec vm_count_by_owner(ne_binary(), api(binary())) -> {non_neg_integer(), non_neg_integer()}.
