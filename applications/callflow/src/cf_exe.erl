...
handle_cast({'callid_update', NewCallId}, #state{call=Call}=State) ->
    ...
    gproc:unreg({'p', 'l', {'cf_exe', PrevCallId}}),
    gproc:reg({'p', 'l', {'cf_exe', NewCallId}}),
...
handle_cast('initialize', #state{call=Call}=State) ->
    ...
    {'noreply', launch_cf_module(State#state{call=CallWithHelpers
                                            ,flow=Flow
                                            ,status='running'
                                            })};
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_event(kz_call_event:doc(), state()) -> gen_server:handle_event_return().
handle_event(JObj, #state{cf_module_pid=PidRef
    ...
    case {kz_util:get_event_type(JObj), kz_call_event:call_id(JObj)} of
        ...
        {{<<"call_event">>, <<"CHANNEL_DIRECT">>}, _} ->
            lager:debug("HANDLE CHANNEL_DIRECT"),
