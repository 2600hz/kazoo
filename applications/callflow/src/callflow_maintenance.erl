...
account_set_classifier_inherit(Classifier, Account) ->
    {'ok', AccountDb} = kapps_util:get_accounts_by_name(kz_account:normalize_name(Account)),
...
list_account_restrictions(Account) ->
    {'ok', AccountDb} = kapps_util:get_accounts_by_name(kz_account:normalize_name(Account)),
    DbNameEncoded = kz_account:format_id(AccountDb,'encoded'),
    io:format("\nAccount level classifiers:\n\n"),
    print_call_restrictions(DbNameEncoded, kz_account:format_id(AccountDb,'raw')),
...
update_feature_codes(Account) ->
    AccountDb = kz_account:format_db(Account),
