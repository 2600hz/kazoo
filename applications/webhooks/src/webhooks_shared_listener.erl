...
handle_event(JObj, _State) ->
    ...
    ShouldHandle = kz_api:is_federated_event(JObj)
        andalso not lists:member(kz_api:event_zone(JObj), WHZones)
        andalso kz_util:to_binary(kz_nodes:local_zone()) == hd(WHZones),
    Options = [{<<"should_handle_federated">>, ShouldHandle}],
    {'reply', Options}.
