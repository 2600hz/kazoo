...
`gateways.[].custom_sip_headers./^[a-zA-z0-9_\-]+$/` | The X- SIP header to add | `string()` |   | `false`
`gateways.[].custom_sip_headers.in` | Custom SIP Headers to be applied to calls inbound to Kazoo from the device | [#/definitions/custom_sip_headers](#custom_sip_headers) |   | `false`
`gateways.[].custom_sip_headers.out` | Custom SIP Headers to be applied to calls outbound from Kazoo to the device | [#/definitions/custom_sip_headers](#custom_sip_headers) |   | `false`
