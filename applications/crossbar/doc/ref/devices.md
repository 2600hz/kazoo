...
`name` | A friendly name for the device | `string` |   | `true`
`outbound_flags` | List of flags (features) this device requires when making outbound calls | `array` |   | `false`
`outbound_flags.[]` |   | `string` |   | `false`
`owner_id` | The ID of the user object that 'owns' the device | `string` |   | `false`
...
`sip.invite_format` | The SIP request URI invite format | `string` | `username` | `false`
`sip.ip` | IP address for this device | `string` |   | `false`
`sip.method` | Method of authentication | `string` | `password` | `false`
`sip.number` | The number used if the invite format is 1npan, npan, or e164 (if not set the dialed number is used) | `string` |   | `false`
`sip.password` | SIP authentication password | `string` |   | `false`
...
`sip.username` | SIP authentication username | `string` |   | `false`
...
```curl
curl -v http://{SERVER}:8000//v2/accounts/{ACCOUNTID}/devices
```

#### Create

> PUT /v2/accounts/{ACCOUNTID}/devices
...
> DELETE /v2/accounts/{ACCOUNTID}/devices/{ID}

```curl
curl -v http://{SERVER}:8000//v2/accounts/{ACCOUNTID}/devices/{ID}
```

#### Fetch

> GET /v2/accounts/{ACCOUNTID}/devices/{ID}
...
> POST /v2/accounts/{ACCOUNTID}/devices/{ID}
...
> GET /v2/accounts/{ACCOUNTID}/devices/status

```curl
curl -v http://{SERVER}:8000//v2/accounts/{ACCOUNTID}/devices/status
```

#### Change

> POST /v2/accounts/{ACCOUNTID}/devices/{DEVICEID}/sync

```curl
curl -v http://{SERVER}:8000//v2/accounts/{ACCOUNTID}/devices/{DEVICEID}/sync
```

#### Fetch

> GET /v2/accounts/{ACCOUNTID}/devices/{ID}/quickcall/{ID}

```curl
curl -v http://{SERVER}:8000//v2/accounts/{ACCOUNTID}/devices/{ID}/quickcall/{ID}
```
