...
post(Context, ?FIX, Num) ->
    ...
    Result = kazoo_number_manager_maintenance:copy_single_number_to_account_db(Num, AccountDb),
...
post(Context, ?FIX) ->
    AccountDb = cb_context:account_db(Context),
    'ok' = kazoo_number_manager_maintenance:fix_account_db_numbers(AccountDb),
    summary(Context);
post(Context, ?CHECK) ->
    Numbers = cb_context:req_value(Context, ?COLLECTION_NUMBERS),
