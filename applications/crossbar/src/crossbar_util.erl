...
get_account_realm(AccountId) when is_binary(AccountId) ->
    get_account_realm(kz_account:format_id(AccountId, 'encoded'), AccountId);
...
get_account_doc(<<_/binary>> = Id) ->
    get_account_doc(kz_account:format_id(Id, 'encoded')
                    ,kz_account:format_id(Id, 'raw')
                   ).
...
flush_registrations(Context) ->
    flush_registrations(kz_account:do_get_realm(cb_context:account_id(Context))).
...
flush_registration(Context) ->
    ...
    AccountId = cb_context:account_id(Context),
    Realm = kz_account:do_get_realm(AccountId),
...
update_descendants_tree([Descendant|Descendants], Tree, NewResellerId) ->
    AccountId = kz_account:format_id(Descendant, 'raw'),
    AccountDb = kz_account:format_id(AccountId, 'encoded'),
...
get_token_restrictions(AuthModule, AccountId, OwnerId) ->
    case kz_account:is_it_system_admin(AccountId) of
        'true' -> 'undefined';
        'false' ->
            Restrictions =
...
maybe_refresh_fs_xml('user', Context, 'true') ->
    ...
    Realm = kz_account:do_get_realm(AccountDb),
    Id = kz_doc:id(Doc),
...
maybe_refresh_fs_xml('device', Context, Precondition) ->
    ...
    ( Precondition
      ...
      or (kz_json:is_true(<<"enabled">>, DbDoc) andalso
        ...
        refresh_fs_xml(
          kz_account:do_get_realm(cb_context:account_db(Context))
          ,DbDoc
         ),
    'ok';
...
refresh_fs_xml(Context) ->
    Realm = kz_account:do_get_realm(cb_context:account_db(Context)),
    DbDoc = cb_context:fetch(Context, 'db_doc'),
