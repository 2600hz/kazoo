...
%% @end
%%--------------------------------------------------------------------
-spec response(fails(), kz_json:key(), cb_context:context()) ->
                      cb_context:context().
...
%% @end
%%--------------------------------------------------------------------
-spec response(fails(), kz_json:key(), maybe(integer()), cb_context:context()) ->
                      cb_context:context().
...
%% @end
%%--------------------------------------------------------------------
-spec response(fails(), kz_json:key(), maybe(integer()), kz_json:json_term(), cb_context:context()) -> cb_context:context().
...
%% @end
%%--------------------------------------------------------------------
-spec create_response(crossbar_status(), kz_json:key(), maybe(integer())
...
%% @end
%%--------------------------------------------------------------------
-spec get_account_realm(ne_binary() | cb_context:context()) -> maybe(binary()).
...
-spec get_account_doc(ne_binary()) -> maybe(kz_json:object()).
-spec get_account_doc(ne_binary(), ne_binary()) -> maybe(kz_json:object()).
...
-spec flush_registration(maybe(binary()), ne_binary() | cb_context:context()) -> 'ok'.
...
-spec maybe_flush_registration_on_password(maybe(binary()), kz_json:object(), kz_json:object()) -> 'ok'.
maybe_flush_registration_on_password(Realm, OldDevice, NewDevice) ->
...
-spec maybe_flush_registration_on_username(maybe(binary()), kz_json:object(), kz_json:object()) -> 'ok'.
...
-spec maybe_flush_registration_on_ownerid(maybe(binary()), kz_json:object(), kz_json:object()) -> 'ok'.
...
-spec maybe_flush_registration_on_enabled(maybe(binary()), kz_json:object(), kz_json:object()) -> 'ok'.
...
%% @end
%%--------------------------------------------------------------------
-spec move_service(ne_binary(), ne_binaries(), ne_binary(), maybe(boolean())) ->
                          {'ok', kz_json:object()} |
                          {'error', any()}.
...
-spec move_service_doc(ne_binaries(), ne_binary(), maybe(boolean()), kz_json:object()) ->
                          {'ok', kz_json:object()} |
                          {'error', any()}.
move_service_doc(NewTree, NewResellerId, Dirty, JObj) ->
    PreviousTree = kz_account:tree(JObj),
...
-spec filter_by_account_id(kz_json:object(), ne_binaries(), ne_binary()) -> ne_binaries().
...
%% @end
%%--------------------------------------------------------------------
-spec disable_account(maybe(binary())) -> 'ok' | {'error', any()}.
...
%% @end
%%--------------------------------------------------------------------
-spec enable_account(maybe(binary())) -> 'ok' | {'error', any()}.
...
-spec response_auth(kz_json:object()) ->
                           kz_json:object().
-spec response_auth(kz_json:object(), maybe(binary())) ->
                           kz_json:object().
-spec response_auth(kz_json:object(), maybe(binary()), maybe(binary())) ->
                           kz_json:object().
response_auth(JObj) ->
    response_auth(JObj
...
-spec populate_resp(kz_json:object(), maybe(binary()), maybe(binary())) -> kz_json:object().
...
-spec get_language(ne_binary(), maybe(binary())) -> ne_binary().
...
-spec get_user_timezone(maybe(binary()), maybe(binary())) -> maybe(binary()).
...
-spec get_account_timezone(maybe(binary())) -> maybe(binary()).
...
-spec apply_response_map_item({kz_json:key(), kz_json:key() | fun()}, kz_json:object(), kz_json:object()) ->
                                     kz_json:object().
...
-spec get_priv_level(ne_binary(), ne_binary()) -> maybe(binary()).
...
-spec get_system_token_restrictions(atom()) -> maybe(kz_json:object()).
...
-spec get_account_token_restrictions(ne_binary(), atom()) -> maybe(kz_json:object()).
...
-spec get_priv_level_restrictions(maybe(kz_json:object()), ne_binary()) -> maybe(kz_json:object()).
...
-spec server_changed(kz_json:object(), maybe(kz_json:object())) -> boolean().
...
%% @public
-spec get_devices_by_owner(ne_binary(), maybe(binary())) -> ne_binaries().
