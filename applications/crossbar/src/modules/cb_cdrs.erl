...
to_csv(Req, Context, {ViewName, Options0}) ->
    ...
    crossbar_view:send_chunked_modb(cb_context:store(Context, 'is_csv', 'true'), Req, ViewName, Options).
...
get_view_options([{<<"cdrs">>, [?PATH_INTERACTION]}, {?KZ_ACCOUNTS_DB, _}|_]) ->
    ...
    ,props:filter_undefined(
       [{'start_key_map', []}
       ,{'end_key_map', crossbar_view:suffix_key_fun([kz_json:new()])}
       ,{'group', 'true'}
       ,{'group_level', 2}
       ,{'reduce', 'true'}
...
get_view_options([{<<"cdrs">>, [?PATH_INTERACTION]}, {<<"users">>, [OwnerId]}|_]) ->
    ...
    ,props:filter_undefined(
       [{'start_key_map', [OwnerId]}
       ,{'end_key_map', fun(Ts) -> [OwnerId, Ts, kz_json:new()] end}
       ,{'group', 'true'}
       ,{'group_level', 3}
       ,{'reduce', 'true'}
        | maybe_add_stale_to_options(?STALE_CDR)
