...
maybe_authenticate_user(Context, Credentials, <<"md5">>, ?NE_BINARY = Account, OTP) ->
...
maybe_authenticate_user(Context, Credentials, <<"sha">>, ?NE_BINARY = Account, OTP) ->
    AccountDb = kz_util:format_account_id(Account, 'encoded'),
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            Context2 = normalize_results(Context1, cb_context:doc(Context1), <<"sha">>),
            maybe_auth_otp(Context2, OTP);
        _Status ->
            lager:debug("credentials do not belong to any user"),
            cb_context:add_system_error('invalid_credentials', Context)
    end;
maybe_authenticate_user(Context, _Creds, _Method, _Account, _OTP) ->
