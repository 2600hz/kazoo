...
maybe_authenticate_user(Context) ->
    ...
    AccountName = kz_account:normalize_name(kz_json:get_value(<<"account_name">>, JObj)),
...
maybe_account_is_expired(Context, Account) ->
    case kz_account:is_it_expired(Account) of
        'false' -> maybe_account_is_enabled(Context, Account);
        {'true', Expired} ->
            _ = kz_util:spawn(fun kz_account:do_maybe_disable/1, [Account]),
            Cause =
                kz_json:from_list(
...
reset_id(?MATCH_ACCOUNT_ENCODED(A,B,Rest)) ->
    Noise = kz_term:rand_hex_binary((?RESET_ID_SIZE - 32) / 2),
...
reset_id(<<ResetId:?RESET_ID_SIZE/binary>>) ->
    ...
    kz_account:format_db(kz_term:to_lower_binary(Account)).
