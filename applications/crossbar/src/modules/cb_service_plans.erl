...
validate(Context, ?AVAILABLE) ->
    ...
    ResellerDb = kz_account:format_id(ResellerId, 'encoded'),
...
validate(Context, ?OVERRIDE) ->
    ...
    case kz_account:is_it_system_admin(AuthAccountId) of
        'true' ->
            crossbar_doc:load(
...
is_allowed(Context) ->
    ...
    (AuthAccountId =:= ResellerId
     orelse kz_account:is_it_system_admin(AuthAccountId)
    )
