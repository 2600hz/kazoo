...
init() ->
    ...
    crossbar_bindings:bind(<<"*.execute.delete.temporal_rules_sets">>, ?MODULE, 'delete').
...
validate_patch(Id, Context) ->
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            ...
            TmpRulesSetsJObj = wh_json:merge_jobjs(PatchJObj, cb_context:doc(Context1)),

            lager:debug("patched doc, now validating"),
            OnSuccess = fun(C) -> on_successful_validation(Id, C) end,
            cb_context:validate_request_data(<<"temporal_rules_sets">>, cb_context:set_req_data(Context, TmpRulesSetsJObj), OnSuccess);
        _Status ->
            Context1
    end.
