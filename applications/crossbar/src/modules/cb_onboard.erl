                  ...
                  ,fun(J) ->
                           case kz_device:sip_username(J) of
                               'undefined' ->
                                   ...
                                   kz_device:set_sip_username(J, list_to_binary(["user_", kz_term:rand_hex_binary(Strength)]));
                               _ ->
                                   J
                           end
                   end
                  ,fun(J) ->
                           case kz_device:sip_password(J) of
                               'undefined' ->
                                   ...
                                   kz_device:set_sip_password(J, kz_term:rand_hex_binary(Strength));
                               _ ->
                                   J
                           end
                   end
...
create_exten_callflow(JObj, Iteration, Context, {Pass, Fail}) ->
    ...
    Generators = [fun(J) ->
                          ...
                          DefaultFlow = kapps_config:get_string(?OB_CONFIG_CAT, <<"default_extension_callflow">>
                                                                     ,kz_term:to_binary(?DEFAULT_FLOW)),
