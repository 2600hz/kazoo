...
maybe_check_port_requests(Context) ->
    ...
    of
        {'true', 'false'} ->
            %% Reseller is handling port request so not bother the user with port request alerts.
            Context;
        {'true', 'true'} ->
...
-spec is_reseller_handling_port_requests(kz_term:api_ne_binary()) -> boolean().
is_reseller_handling_port_requests('undefined') ->
    'false';
is_reseller_handling_port_requests(ResellerId) ->
    is_reseller_handling_port_requests(kzd_whitelabel:fetch(ResellerId), ResellerId).

-spec is_reseller_handling_port_requests({'ok', kz_json:object()} | {'error', any()}, kz_term:api_ne_binary()) ->
                                             boolean().
is_reseller_handling_port_requests({'ok', Whitelabel}, ResellerId) ->
    kzd_whitelabel:port_authority(Whitelabel) =:= ResellerId;
is_reseller_handling_port_requests({'error', 'not_found'}, _ResellerId) ->
    'false'.
