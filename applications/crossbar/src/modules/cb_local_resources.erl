...
validate_request(ResourceId, Context) ->
    ...
    case lists:any(fun is_registering_gateway/1, Gateways) of
        'true' ->
...
check_if_peer(ResourceId, Context) ->
    ...
    of
        ...
        {'true', 'false'} ->
            ...
            C = cb_context:add_validation_error([<<"peer">>], <<"forbidden">>, Msg, Context),
...
validate_gateway_ips([{Idx, 'undefined', 'undefined'}|IPs], SIPAuth, ACLs, ResourceId, Context, 'success') ->
    ...
    Property = [<<"gateways">>, Idx, <<"server">>],
    C = cb_context:add_validation_error(Property, <<"required">>, Msg, Context),
...
validate_gateway_ips([{Idx, 'undefined', ServerIP}|IPs], SIPAuth, ACLs, ResourceId, Context, 'success') ->
    case kz_network_utils:is_ipv4(ServerIP) of
        false ->
            NewIPs = [{Idx, 'undefined', 'undefined'} | IPs],
            validate_gateway_ips(NewIPs, SIPAuth, ACLs, ResourceId, Context, cb_context:resp_status(Context));
        'true' ->
            case validate_ip(ServerIP, SIPAuth, ACLs, ResourceId) of
                ...
                'false' ->
                    ...
                    Property = [<<"gateways">>, Idx, <<"server">>],
                    C = cb_context:add_validation_error(Property, <<"unique">>, Msg, Context),
                    validate_gateway_ips(IPs, SIPAuth, ACLs, ResourceId, C, cb_context:resp_status(C))
            end
    end;
...
validate_gateway_ips([{Idx, InboundIP, ServerIP}|IPs], SIPAuth, ACLs, ResourceId, Context, 'success') ->
    case kz_network_utils:is_ipv4(InboundIP) of
        'false' ->
            NewIPs = [{Idx, 'undefined', ServerIP} | IPs],
            validate_gateway_ips(NewIPs, SIPAuth, ACLs, ResourceId, Context, cb_context:resp_status(Context));
        'true' ->
            case validate_ip(InboundIP, SIPAuth, ACLs, ResourceId) of
                ...
                'false' ->
                    ...
                    Property = [<<"gateways">>, Idx, <<"inbound_ip">>],
                    C = cb_context:add_validation_error(Property, <<"unique">>, Msg, Context),
                    validate_gateway_ips(IPs, SIPAuth, ACLs, ResourceId, C, cb_context:resp_status(C))
            end
    end.
...
maybe_remove_aggregate(ResourceId, Context) ->
    case success =:= cb_context:resp_status(Context)
        andalso kz_datamgr:open_doc(?KZ_SIP_DB, ResourceId)
    of
        false -> false;
...
get_all_sip_auth_ips() ->
    case kz_datamgr:get_results(?KZ_SIP_DB, <<"credentials/lookup_by_ip">>) of
        {error, _R} -> [];
        {ok, JObjs} ->
            [{kz_json:get_value(<<"key">>, JObj), kz_doc:id(JObj)} || JObj <- JObjs]
    end.
...
get_all_acl_ips() ->
    ...
    Resp = kapps_util:amqp_pool_request(Req
    ...
    case Resp of
        {error, _} -> [];
        {ok, RespJObj} ->
            ValueJObj = kz_json:get_value(<<"Value">>, RespJObj, kz_json:new()),
            [{CIDR, kz_json:get_value(<<"authorizing_id">>, JObj)}
             || {_, JObj} <- kz_json:to_proplist(ValueJObj),
                CIDR <- [kz_json:get_ne_binary_value(<<"cidr">>, JObj)],
                CIDR =/= undefined
            ]
    end.
...
maybe_remove_aggregates([Resource|Resources]) ->
    _ = case kz_datamgr:open_doc(?KZ_SIP_DB, kz_doc:id(Resource)) of
            {error, not_found} -> ok;
            {ok, JObj} ->
                kz_datamgr:del_doc(?KZ_SIP_DB, JObj),
                _ = kapi_switch:publish_reload_gateways(),
