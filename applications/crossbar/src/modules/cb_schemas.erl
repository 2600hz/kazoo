...
-spec authorize_on_nouns(req_nouns()) -> 'true'.
authorize(Context) ->
    authorize_on_nouns(cb_context:req_nouns(Context)).

authorize_on_nouns([{<<"schemas">>,_}]) ->
    lager:debug("authorizing request to fetch schema(s)"),
    'true'.

-spec authenticate(cb_context:context()) -> 'true'.
-spec authenticate_on_nouns(req_nouns()) -> 'true'.
authenticate(Context) ->
    authenticate_on_nouns(cb_context:req_nouns(Context)).

authenticate_on_nouns([{<<"schemas">>,_}]) ->
...
validate(Context, Id) ->
    read(Id, cb_context:set_account_id(Context, ?WH_SCHEMA_DB)).

validate(Context, Id, ?VALIDATION_PATH_TOKEN) ->
    OnSuccess = fun(C) -> cb_context:set_resp_data(C, cb_context:doc(C)) end,
