...
authenticate_nouns([{<<"google_auth">>, [<<"recovery">>]}]) -> 'true';
...
maybe_authenticate_user(Context) ->
	...
	lager:info("maybe_authenticate_user ~p", [JObj]),
	case kazoo_oauth_client:authenticate(JObj) of
		{'ok', OAuth} ->
			lager:info("Verified OAuth ~p",[OAuth]),
			maybe_account_is_disabled(Context#cb_context{doc=OAuth,resp_status='success'});

		{'error', Error} ->
			lager:info("Error Verifying Token ~p",[Error]),
...
create_token(Context) ->
	...
	case wh_json:get_value(<<"User">>, JObj) of
		'undefined' -> crossbar_util:response(wh_json:set_value(<<"profile">>, wh_json:get_value(<<"Profile">>, JObj), wh_json:new()),Context);
		User -> create_auth_token(Context)
	end.
	
	
create_auth_token(Context) ->
    JObj = cb_context:doc(Context),
	lager:info("create_auth_token ~p", [JObj]),
	lager:info("create_auth_token User ~p",[wh_json:get_value(<<"User">>, JObj)]),
