...
change_in_servers_test_() ->
    ...
    [{"Verifying multiple changes results in a change required"
     ...
     |
     [{iolist_to_binary(
         io_lib:format("Verifying changing ~s results in a change required", [wh_util:join_binary(K)])
        )
...
change_server([<<"options">>, <<"enabled">>] = Key, Servers) ->
    wh_json:set_value([1 | Key], 'false', Servers);
change_server(Key, Servers) ->
    New = wh_util:rand_hex_binary(5),
    wh_json:set_value([1 | Key], New, Servers).
