...
%% API
-export([create_config/0,amqperr/1,start/0,kazoo_ver/1,get_oid/4]).
...
get_oid(get_next,RowIndex,Cols,Table) ->
    io:format("Table: ~p Row: ~p Cols: ~p~n",[Table,RowIndex, Cols]),
...
%% @public
%% @doc
%% Return the number of errors in amqp channels
%% @end
-spec amqperr('get') -> {'value',integer()}.
amqperr(get) ->
    {value, 200}.
...
create_config() ->
    ...
    write_conf(agent_entry,write_agent_config,ADir,
	       [[intAgentUDPPort, 4000], [snmpEngineID,"Kazoo agent"],
		[snmpEngineMaxMessageSize, 484],[intAgentIpAddress,[0,0,0,0]]
	       ]),
    write_conf(standard_entry,write_standard_config,ADir,
	       [[sysDescr,"Erlang/OTP Agent"],[sysName,"Kazoo agent"],
		[sysContact,"info@2600hz.com"],[sysLocation,"US"],
		[sysObjectID,[3,6,1,4,1,193,19]],
		[sysServices, 72], [snmpEnableAuthenTraps, disabled]]),
    ...
    write_conf(vacm_acc_entry,append_vacm_config,ADir,
		 ...
		 "restricted"],
		["initial", "", usm, authNoPriv, exact, "internet", "internet",
		 "internet"],
		["initial", "", usm, authPriv, exact, "internet", "internet", 
		 "internet"]]),
...
write_conf(EMod,EFun,WFun,Dir,EList) ->
	...
	true ->
	    io:format("Writing to ~s ~s~n~p~n",[WFun,Dir,Entries]),
