...
filter_req(Prop) when is_list(Prop) ->
    case filter_req_v(Prop) of
        'true' -> wh_api:build_message(Prop, ?FILTER_REQ_HEADERS, ?OPTIONAL_FILTER_REQ_HEADERS);
        'false' -> {'error', "Proplist failed validation for filter req"}
    end;
filter_req(JObj) ->
    filter_req(wh_json:to_proplist(JObj)).

-spec filter_req_v(api_terms()) -> boolean().
filter_req_v(Prop) when is_list(Prop) ->
    wh_api:validate(Prop, ?FILTER_REQ_HEADERS, ?FILTER_REQ_VALUES, ?FILTER_REQ_TYPES);
filter_req_v(JObj) ->
    filter_req_v(wh_json:to_proplist(JObj)).

-spec filter_resp(api_terms()) -> {'ok', iolist()} | {'error', string()}.
filter_resp(Prop) when is_list(Prop) ->
    case filter_resp_v(Prop) of
        'true' -> wh_api:build_message(Prop, ?FILTER_RESP_HEADERS, ?OPTIONAL_FILTER_RESP_HEADERS);
        'false' -> {'error', "Proplist failed validation for filter resp"}
    end;
filter_resp(JObj) ->
    filter_resp(wh_json:to_proplist(JObj)).

-spec filter_resp_v(api_terms()) -> boolean().
filter_resp_v(Prop) when is_list(Prop) ->
    wh_api:validate(Prop, ?FILTER_RESP_HEADERS, ?FILTER_RESP_VALUES, ?FILTER_RESP_TYPES);
filter_resp_v(JObj) ->
    filter_resp_v(wh_json:to_proplist(JObj)).
...
publish_filter_req(Req, ContentType) ->
    {'ok', Payload} = wh_api:prepare_api_payload(Req, ?FILTER_REQ_VALUES, fun ?MODULE:filter_req/1),
...
publish_filter_resp(RespQ, JObj, ContentType) ->
    {'ok', Payload} = wh_api:prepare_api_payload(JObj, ?FILTER_RESP_VALUES, fun filter_resp/1),
    amqp_util:targeted_publish(RespQ, Payload, ContentType).
