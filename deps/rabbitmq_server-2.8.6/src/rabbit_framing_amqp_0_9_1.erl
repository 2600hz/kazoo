...
-type(amqp_method_field_name() ::
       ( global | client_properties | mechanism | response
       | locale | consumer_tag | challenge | consumer_tag
       | exchange | ticket | cluster_id | requeue
       | response | routing_key | channel_max | frame_max
       | heartbeat | delivery_tag | message_count | channel_max
       | frame_max | heartbeat | routing_key | exchange
       | virtual_host | redelivered | multiple | capabilities
       | insist | routing_key | delivery_tag | delivery_tag
       | known_hosts | ticket | exchange | reply_code
       | consumer_tag | delivery_tag | reply_text | class_id
       | method_id | queue | no_ack | routing_key
       | out_of_band | ticket | delivery_tag | channel_id
       | exchange | active | message_count | ticket
       | queue | active | nowait | nowait
       | reply_code | reply_text | class_id | method_id
       | exchange | queue | multiple | realm
       | exclusive | passive | arguments | no_local
       | active | write | read | version_minor
       | server_properties | mechanisms | requeue | arguments
       | locales | no_ack | ticket | arguments
       | type | immediate | passive | mandatory
       | exclusive | durable | auto_delete | internal
       | nowait | nowait | ticket | nowait
       | message_count | exchange | if_unused | nowait
       | queue | requeue | if_unused | ticket
       | queue | destination | source | requeue
       | routing_key | nowait | arguments | version_major
       | ticket | ticket | destination | reply_code
       | source | ticket | routing_key | redelivered
       | nowait | arguments | reply_text | ticket
       | prefetch_size | ticket | queue | passive
       | exchange | durable | routing_key | exclusive
       | auto_delete | nowait | arguments | consumer_tag
       | if_empty | queue | message_count | consumer_count
       | arguments | nowait | consumer_tag | ticket
       | prefetch_count | queue | exchange | routing_key
       | nowait )).
