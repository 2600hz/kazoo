...
-type(amqp_method_field_name() ::
       ( ticket | routing_key | channel_max | frame_max
       | heartbeat | delivery_tag | ticket | integer_2
       | exchange | integer_result | capabilities | ticket
       | content_size | arguments | insist | version_minor
       | message_count | known_hosts | integer_op | mandatory
       | ticket | consumer_tag | known_hosts | string_2
       | server_properties | passive | reply_code | exclusive
       | auto_delete | consumer_tag | nowait | arguments
       | requeue | consumer_tag | operation | requeue
       | passive | queue | message_count | mandatory
       | consumer_count | routing_key | active | delivery_tag
       | identifier | active | ticket | delivery_tag
       | queue | exchange | nowait | routing_key
       | durable | result | reply_text | details
       | multiple | virtual_host | write | reply_code
       | global | string_op | reply_text | class_id
       | content_checksum | consume_rate | method_id | multiple
       | queue | if_unused | queue | nowait
       | integer_3 | version_major | realm | exclusive
       | exchange | mechanisms | reply_code | locales
       | ticket | active | nowait | ticket
       | mechanism | read | redelivered | if_unused
       | if_empty | locale | cluster_id | prefetch_count
       | immediate | response | ticket | message_count
       | operation | type | frame_max | passive
       | delivery_tag | ticket | auto_delete | reply_code
       | reply_text | queue | internal | nowait
       | nowait | arguments | no_ack | exclusive
       | prefetch_count | delivery_tag | nowait | nowait
       | staged_size | consumer_tag | integer_4 | reply_text
       | ticket | prefetch_size | arguments | host
       | challenge | global | no_local | prefetch_size
       | prefetch_count | no_ack | routing_key | routing_key
       | consumer_tag | global | queue | exchange
       | ticket | consumer_tag | string_1 | queue
       | consumer_tag | exchange | no_local | redelivered
       | no_ack | exclusive | exchange | nowait
       | nowait | response | no_local | exclusive
       | consumer_tag | ticket | exchange | durable
       | routing_key | consumer_tag | nowait | prefetch_size
       | dtx_identifier | immediate | identifier | consumer_tag
       | consumer_tag | reply_code | reply_text | out_of_band
       | channel_max | integer_1 | exchange | ticket
       | client_properties | exchange | message_count | identifier
       | routing_key | consumer_tag | mandatory | immediate
       | class_id | queue | consumer_tag | ticket
       | string_result | delivery_tag | reply_code | meta_data
       | reply_text | exchange | requeue | routing_key
       | exchange | method_id | exchange | heartbeat
       | queue | exchange | consumer_tag | delivery_tag
       | table | result | redelivered | consumer_tag
       | exchange | delivery_tag | routing_key | routing_key
       | routing_key | requeue | nowait | queue
       | ticket | queue )).
