...
-type(amqp_method_field_name() ::
       ( arguments | exchange | consumer_tag | routing_key
       | multiple | reply_text | consumer_tag | virtual_host
       | exchange | meta_data | auto_delete | routing_key
       | routing_key | arguments | result | nowait
       | routing_key | mandatory | server_properties | write
       | dtx_identifier | reply_text | reply_code | exchange
       | exclusive | active | capabilities | delivery_tag
       | exclusive | no_local | immediate | queue
       | method_id | redelivered | requeue | if_unused
       | exchange | operation | internal | global
       | out_of_band | consumer_tag | heartbeat | nowait
       | realm | passive | delivery_tag | ticket
       | consume_rate | integer_3 | class_id | active
       | routing_key | ticket | exchange | exchange
       | exchange | consumer_tag | queue | exchange
       | global | delivery_tag | passive | ticket
       | requeue | class_id | identifier | frame_max
       | insist | consumer_tag | reply_text | nowait
       | ticket | requeue | nowait | challenge
       | ticket | locales | queue | ticket
       | method_id | reply_code | delivery_tag | table
       | reply_code | mechanisms | delivery_tag | message_count
       | read | operation | reply_code | routing_key
       | exclusive | identifier | staged_size | consumer_tag
       | prefetch_size | ticket | queue | message_count
       | locale | ticket | exchange | arguments
       | routing_key | arguments | nowait | message_count
       | consumer_tag | integer_1 | ticket | queue
       | consumer_tag | queue | channel_max | content_size
       | nowait | queue | exchange | string_2
       | exchange | consumer_count | exclusive | identifier
       | reply_text | consumer_tag | ticket | ticket
       | consumer_tag | known_hosts | reply_text | multiple
       | frame_max | response | routing_key | redelivered
       | response | delivery_tag | routing_key | result
       | consumer_tag | client_properties | prefetch_size | nowait
       | nowait | delivery_tag | prefetch_count | consumer_tag
       | details | no_ack | consumer_tag | string_op
       | message_count | no_local | exclusive | auto_delete
       | known_hosts | global | immediate | integer_2
       | content_checksum | no_ack | prefetch_count | queue
       | integer_op | exchange | integer_result | prefetch_size
       | heartbeat | consumer_tag | routing_key | passive
       | if_unused | routing_key | ticket | mechanism
       | version_major | consumer_tag | reply_code | queue
       | queue | no_ack | ticket | immediate
       | active | type | requeue | nowait
       | exchange | integer_4 | string_result | mandatory
       | string_1 | host | redelivered | durable
       | ticket | channel_max | nowait | durable
       | mandatory | nowait | if_empty | queue
       | exchange | version_minor | delivery_tag | reply_code
       | nowait | cluster_id | ticket | prefetch_count
       | no_local | reply_text )).
