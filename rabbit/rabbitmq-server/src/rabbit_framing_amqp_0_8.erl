...
-type(amqp_method_field_name() ::
       ( arguments | exchange | integer_op | locale
       | consumer_tag | routing_key | routing_key | routing_key
       | virtual_host | integer_4 | auto_delete | routing_key
       | reply_code | queue | queue | nowait
       | ticket | ticket | server_properties | ticket
       | mandatory | requeue | reply_text | exchange
       | queue | capabilities | exchange | no_local
       | integer_result | string_op | redelivered | consume_rate
       | queue | ticket | nowait | operation
       | staged_size | nowait | if_unused | exchange
       | details | heartbeat | delivery_tag | no_ack
       | consumer_tag | internal | immediate | cluster_id
       | passive | prefetch_count | no_local | reply_text
       | class_id | active | string_result | queue
       | exchange | consumer_tag | queue | exchange
       | if_empty | consumer_tag | passive | exchange
       | meta_data | class_id | routing_key | exchange
       | insist | routing_key | reply_text | nowait
       | method_id | channel_max | challenge | reply_code
       | consumer_tag | global | locales | frame_max
       | consumer_tag | exclusive | ticket | immediate
       | reply_code | mechanisms | message_count | redelivered
       | requeue | durable | routing_key | operation
       | result | known_hosts | global | out_of_band
       | message_count | reply_text | requeue | nowait
       | arguments | delivery_tag | ticket | message_count
       | consumer_tag | dtx_identifier | ticket | exchange
       | write | exchange | consumer_tag | queue
       | ticket | delivery_tag | nowait | ticket
       | client_properties | consumer_count | message_count | identifier
       | ticket | exclusive | consumer_tag | reply_text
       | routing_key | exclusive | requeue | nowait
       | known_hosts | frame_max | response | delivery_tag
       | result | response | reply_code | delivery_tag
       | read | prefetch_size | delivery_tag | no_local
       | nowait | arguments | no_ack | integer_3
       | exclusive | prefetch_size | multiple | queue
       | auto_delete | realm | ticket | exchange
       | routing_key | arguments | nowait | prefetch_count
       | active | ticket | integer_1 | heartbeat
       | delivery_tag | queue | mandatory | passive
       | if_unused | table | content_size | mechanism
       | queue | exchange | exchange | version_major
       | consumer_tag | identifier | nowait | string_2
       | identifier | consumer_tag | active | type
       | exchange | consumer_tag | method_id | string_1
       | reply_text | multiple | mandatory | routing_key
       | redelivered | host | reply_code | ticket
       | channel_max | consumer_tag | nowait | durable
       | delivery_tag | prefetch_count | prefetch_size | consumer_tag
       | ticket | queue | consumer_tag | routing_key
       | version_minor | exclusive | reply_code | nowait
       | global | immediate | ticket | integer_2
       | content_checksum | no_ack )).
