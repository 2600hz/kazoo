...
-type(amqp_method_field_name() ::
       ( ticket | challenge | delivery_tag | ticket
       | arguments | ticket | if_unused | no_local
       | known_hosts | consumer_tag | queue | message_count
       | routing_key | table | version_minor | delivery_tag
       | reply_text | internal | queue | consumer_count
       | identifier | reply_text | consumer_tag | reply_text
       | multiple | routing_key | integer_result | nowait
       | exchange | channel_max | virtual_host | exchange
       | routing_key | write | ticket | reply_code
       | delivery_tag | dtx_identifier | nowait | exchange
       | host | details | identifier | reply_text
       | frame_max | active | queue | auto_delete
       | exchange | known_hosts | nowait | if_empty
       | result | nowait | frame_max | integer_op
       | integer_1 | response | global | consumer_tag
       | message_count | content_size | queue | exchange
       | exchange | ticket | integer_3 | identifier
       | consumer_tag | mandatory | locales | consumer_tag
       | exchange | active | ticket | prefetch_size
       | method_id | arguments | exchange | reply_text
       | string_result | queue | message_count | multiple
       | heartbeat | class_id | meta_data | insist
       | routing_key | passive | read | durable
       | mechanism | prefetch_count | delivery_tag | prefetch_count
       | consumer_tag | consumer_tag | queue | ticket
       | exclusive | ticket | ticket | immediate
       | cluster_id | queue | passive | consumer_tag
       | content_checksum | arguments | consumer_tag | prefetch_size
       | ticket | nowait | routing_key | routing_key
       | ticket | routing_key | consumer_tag | channel_max
       | exclusive | ticket | no_ack | reply_code
       | string_op | nowait | ticket | routing_key
       | exclusive | string_1 | routing_key | requeue
       | response | queue | message_count | exchange
       | delivery_tag | ticket | server_properties | requeue
       | redelivered | consume_rate | queue | no_local
       | consumer_tag | nowait | exchange | type
       | mandatory | result | nowait | auto_delete
       | operation | locale | reply_code | string_2
       | delivery_tag | no_ack | nowait | mechanisms
       | arguments | prefetch_count | mandatory | class_id
       | queue | routing_key | client_properties | consumer_tag
       | routing_key | queue | integer_4 | prefetch_size
       | reply_text | consumer_tag | passive | realm
       | ticket | exchange | capabilities | version_major
       | no_local | heartbeat | active | consumer_tag
       | exchange | requeue | integer_2 | no_ack
       | reply_code | exchange | global | nowait
       | exclusive | delivery_tag | method_id | reply_code
       | operation | immediate | reply_code | exchange
       | consumer_tag | delivery_tag | out_of_band | redelivered
       | requeue | durable | exclusive | nowait
       | if_unused | redelivered | global | staged_size
       | nowait | immediate )).
