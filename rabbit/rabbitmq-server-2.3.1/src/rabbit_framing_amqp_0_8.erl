...
-type(amqp_method_field_name() ::
       ...
       | string_op | consumer_tag | routing_key | meta_data
       | client_properties | ticket | consumer_tag | known_hosts
       | integer_4 | reply_text | exchange | insist
       | routing_key | redelivered | auto_delete | string_result
       | delivery_tag | locale | exchange | nowait
       | message_count | delivery_tag | consumer_tag | reply_text
       | routing_key | consumer_tag | prefetch_size | routing_key
       | active | exclusive | no_local | routing_key
       | operation | global | immediate | integer_2
       | content_checksum | no_ack | passive | ticket
       | integer_op | mandatory | prefetch_size | if_unused
       | consumer_tag | routing_key | immediate | heartbeat
       | arguments | content_size | consumer_tag | nowait
       | mechanisms | ticket | nowait | reply_code
       | queue | class_id | ticket | ticket
       | exchange | queue | requeue | queue
       | exchange | multiple | internal | exchange
       | durable | consumer_tag | redelivered | nowait
       | locales | nowait | nowait | prefetch_count
       | mandatory | integer_3 | consumer_tag | nowait
       | challenge | virtual_host | exchange | active
       | delivery_tag | ticket | prefetch_count | cluster_id
       | version_minor | reply_code | no_local | reply_text
       | out_of_band | method_id | ticket | consumer_tag
       | reply_text | frame_max | reply_text | message_count
       | exchange | dtx_identifier | exchange | arguments
       | passive | queue | routing_key | result
       | message_count | routing_key | write | routing_key
       | channel_max | queue | capabilities | reply_code
       | exchange | nowait | nowait | delivery_tag
       | exclusive | consumer_count | consume_rate | immediate
       | integer_result | consumer_tag | consumer_tag | auto_delete
       | redelivered | requeue | known_hosts | message_count
       | operation | frame_max | global | staged_size
       | type | server_properties | host | queue
       | delivery_tag | ticket | requeue | nowait
       | exclusive | no_ack | ticket | exchange
       | details | consumer_tag | no_local | if_empty
       | consumer_tag | ticket | method_id | prefetch_size
       | ticket | prefetch_count | consumer_tag | reply_text
       | class_id | heartbeat | no_ack | table
       | requeue | nowait | passive | read
       | global | mechanism | exclusive | delivery_tag
       | version_major | routing_key | reply_code | exchange
       | queue | delivery_tag | active | string_1
       | if_unused | identifier | result | exclusive
       | arguments | multiple | queue | ticket
       | consumer_tag | exchange | ticket | routing_key
       | arguments | nowait | durable | response
       | mandatory | integer_1 | ticket | queue
       | queue | realm | reply_code | string_2
       | delivery_tag | response | reply_code | queue
       | exchange | exchange )).
