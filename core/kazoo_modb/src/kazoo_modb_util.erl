...
prev_year_month(<<_/binary>> = Year, Month) ->
    prev_year_month(kz_util:to_integer(Year), Month);
prev_year_month(Year, <<_/binary>> = Month) ->
    prev_year_month(Year, kz_util:to_integer(Month));
...
prev_year_month_bin(Y, M) ->
    ...
    {kz_util:to_binary(Year), kz_util:pad_month(Month)}.
...
split_account_mod(?MATCH_MODB_SUFFIX_RAW(Account,Year,Month)) ->
    ...
    ,kz_util:to_integer(Year)
    ,kz_util:to_integer(Month)
    };
