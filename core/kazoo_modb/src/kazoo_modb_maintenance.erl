...
delete_modbs(Period, ShouldArchive) ->
    ...
    case re:run(Period, Regex, [{'capture', 'all', 'binary'}]) of
        {'match', [_Full, Year, Month]} ->
            delete_modbs(Year, Month, kz_util:is_true(ShouldArchive));
        'nomatch' ->
            io:format("period '~s' does not match YYYYMM format~n", [Period])
    end.

delete_modbs(<<_/binary>> = Year, Month, ShouldArchive) ->
    delete_modbs(kz_util:to_integer(Year), Month, ShouldArchive);
delete_modbs(Year, <<_/binary>> = Month, ShouldArchive) ->
    delete_modbs(Year, kz_util:to_integer(Month), ShouldArchive);
...
fix_value_map(<<"pvt_modified">> = Key, Value) -> {Key, kz_util:rfc1036(Value)};
fix_value_map(<<"pvt_created">> = Key, Value) -> {Key, kz_util:rfc1036(Value)};
