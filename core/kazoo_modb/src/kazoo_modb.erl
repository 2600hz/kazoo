...
get_modb(?MATCH_MODB_SUFFIX_ENCODED(_,_,_) = AccountMODb) ->
    kzs_util:format_account_modb(AccountMODb, 'raw');
...
get_modb(Account, Year, Month) ->
    AccountDb = kzs_util:format_account_db(Account),
    kzs_util:format_account_mod_id(AccountDb, Year, Month).
...
maybe_create_current_modb(?MATCH_MODB_SUFFIX_ENCODED(_, _, _) = AccountMODb, Options) ->
    maybe_create_current_modb(kzs_util:format_account_modb(AccountMODb, 'raw'), Options);
...
maybe_create(?MATCH_MODB_SUFFIX_ENCODED(_, _, _) = AccountMODb) ->
    maybe_create(kzs_util:format_account_modb(AccountMODb, 'raw'));
