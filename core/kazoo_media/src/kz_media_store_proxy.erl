...
-spec credentials(cowboy_req:req()) -> {api(binary()), api(binary()), cowboy_req:req()}.
credentials(Req0) ->
...
-spec credentials_from_header(ne_binary()) -> {api(binary()), api(binary())}.
...
-spec decoded_credentials(ne_binary()) -> {api(binary()), api(binary())}.
decoded_credentials(EncodedCredentials) ->
