...
port_authority(Doc, Default) ->
    ...
    case kz_json:get_ne_binary_value([<<"port">>, <<"authority">>], Doc) of
        ...
        ?MATCH_ACCOUNT_UNENCODED(_)=Db -> kzd_accounts:format_account_id(Db);
        ?MATCH_ACCOUNT_ENCODED(_)=Db -> kzd_accounts:format_account_id(Db);
        ?MATCH_ACCOUNT_encoded(_)=Db -> kzd_accounts:format_account_id(Db);
        _ -> Default
    end.
...
fetch(Account) ->
    AccoundDb = kzd_accounts:format_account_db(Account),
