...
-spec caller_id_name(kz_proplist()) -> api(binary()).
...
-spec caller_id_number(kz_proplist()) -> api(binary()).
...
-spec callee_id_name(kz_proplist()) -> api(binary()).
...
-spec callee_id_number(kz_proplist()) -> api(binary()).
...
-spec dialed_number(kz_proplist()) -> api(binary()).
dialed_number(Props) ->
...
-spec call_id(kz_proplist()) -> api(binary()).
call_id(Props) ->
...
-spec channel_authorized(kz_proplist()) -> api(binary()).
channel_authorized(Props) ->
    ccv(Props, <<"Channel-Authorized">>).

-spec hunt_destination_number(kz_proplist()) -> api(binary()).
...
-spec resource_id(kz_proplist()) -> api(binary()).
resource_id(Props) ->
    ccv(Props, <<"Resource-ID">>).

-spec authorizing_id(kz_proplist()) -> api(binary()).
authorizing_id(Props) ->
    ccv(Props, <<"Authorizing-ID">>).

-spec authorizing_type(kz_proplist()) -> api(binary()).
authorizing_type(Props) ->
    ccv(Props, <<"Authorizing-Type">>).

-spec account_id(kz_proplist()) -> api(binary()).
account_id(Props) ->
    ccv(Props, <<"Account-ID">>).

-spec account_billing(kz_proplist()) -> api(binary()).
account_billing(Props) ->
    ccv(Props, <<"Account-Billing">>).

-spec reseller_id(kz_proplist()) -> api(binary()).
reseller_id(Props) ->
    ccv(Props, <<"Reseller-ID">>).

-spec reseller_billing(kz_proplist()) -> api(binary()).
reseller_billing(Props) ->
    ccv(Props, <<"Reseller-Billing">>).

-spec to_did(kz_proplist()) -> api(binary()).
to_did(Props) ->
...
-spec ccv(kz_proplist(), ne_binary()) -> api(binary()).
ccv(Props, Key) ->
...
-spec transfer_history(kz_proplist()) -> api(binary()) | kz_proplist().
transfer_history(Props) ->
    props:get_value(<<"variable_transfer_history">>, Props).

-spec transfer_source(kz_proplist()) -> api(binary()) | kz_proplist().
transfer_source(Props) ->
    props:get_value(<<"variable_transfer_source">>, Props).

-spec raw_application_name(kz_proplist()) -> api(binary()).
...
-spec application_name(kz_proplist()) -> api(binary()).
...
-spec event_name(kz_proplist()) -> api(binary()).
event_name(Props) ->
    props:get_first_defined([<<"kazoo_event_name">>
                             ,<<"Event-Name">>
                            ], Props).

-spec from_network_ip(kz_proplist()) -> api(binary()).
...
-spec from_network_port(kz_proplist()) -> api(binary()).
...
-spec user_agent(kz_proplist()) -> api(binary()).
user_agent(Props) ->
