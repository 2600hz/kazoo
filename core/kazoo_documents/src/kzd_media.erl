...
-spec tts_text(doc()) -> api_ne_binary().
-spec tts_text(doc(), Default) -> ne_binary() | Default.
tts_text(Doc) ->
    tts_text(Doc, 'undefined').
tts_text(Doc, Default) ->
    kz_json:get_ne_binary_value([<<"tts">>, <<"text">>], Doc, Default).

-spec set_tts_text(doc(), ne_binary()) -> doc().
