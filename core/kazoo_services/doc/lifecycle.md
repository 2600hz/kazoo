...
Kazoo starts a process, `kz_service_sync` that can be configured to periodically reconcile accounts if needed. This is configured in the `system_config/services` doc, under the `sync_services` flag. It defaults to 'false' so background reconciliation will not happen by default.
...
* `POST /v2/accounts/{ACCOUNT_ID}/service_plans/synchronization`
    - This API will do a full account reconcile, persist, *and* will sync with the bookkeeper (basically what `kz_service_sync` does periodically).
    - This can also be run via SUP on the backend: `sup kazoo_services_maintenance sync {ACCOUNT_ID}`
...
When **M** creates **R1**, it applies a service plan (or several) to the account. Let's continue using the SSSP ($1 per device). After flagging the account as a reseller (`sup kazoo_services_maintenance make_reseller {R1_ACCOUNT_ID}`), **R1** now make sub-accounts for itself.
