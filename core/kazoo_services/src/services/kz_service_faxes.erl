...
reconcile(Services) ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"services/faxes">>, ViewOptions) of
        {'error', _R} ->
            lager:debug("unable to get current faxes in service: ~p", [_R]),
            Services;
...
reconcile_faxbox(Services, JObj) ->
    case kz_doc:type(JObj) =:= <<"faxbox">> of
        'false' -> Services;
        'true' ->
            Quantity = kz_services:updated_quantity(?SERVICE_CATEGORY, <<"faxbox">>, Services),
            kz_services:update(?SERVICE_CATEGORY, <<"faxbox">>, Quantity+1, Services)
    end.
