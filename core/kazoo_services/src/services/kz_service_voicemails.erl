...
-define(SERVICE_CATEGORY, <<"voicemails">>).
...
reconcile(Services) ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"services/voicemails">>, ViewOptions) of
        {'error', _R} ->
            lager:debug("unable to get current voicemails in service: ~p", [_R]),
            Services;
...
reconcile(Services, JObj) ->
    Routines = [fun reconcile_mailbox/2],
    lists:foldl(fun(F, S) ->
                        F(S, JObj)
                end
               ,reconcile(Services)
