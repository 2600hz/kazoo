...
-define(HEARTBEAT, 8 * ?MILLISECONDS_IN_SECOND + rand:uniform(10 * ?MILLISECONDS_IN_SECOND)).
-define(EXPIRE_PERIOD, 1 * ?MILLISECONDS_IN_SECOND).
-define(COLLECT_PERIOD, (4 * ?MILLISECONDS_IN_SECOND)).
...
handle_info('expire_nodes', #state{node=ThisNode, tab=Tab}=State) ->
    ...
    _ = [lager:warning("expiring node ~s", [Node]) || #kz_node{node=Node} <- Nodes],
...
handle_info({'heartbeat', Ref}
    ...
    _ = erlang:send_after(Heartbeat - ?COLLECT_PERIOD, self(), {'heartbeat', Reference}),
...
whapp_info_from_json(Key, Info, {[V | V1], [<<"Uptime">> | K1]}) ->
    whapp_info_from_json(Key, Info#whapp_info{uptime=V}, {V1, K1});
