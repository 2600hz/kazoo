...
-type audio_converted() :: {'ok', any()}|
                           {'error', any()}.

-type audio_convert_funs() :: [fun((kz_term:ne_binary(), map()) -> audio_converted())].
...
%%------------------------------------------------------------------------------
%% @doc Collects the audio related metadata from a file
%%      Now not implemented
%% @end
%%------------------------------------------------------------------------------
...
%% @end
%%------------------------------------------------------------------------------
-spec eval_format(kz_term:ne_binary(), kz_term:ne_binary()) -> audio_convert_funs() | {'error', kz_term:ne_binary()}.
eval_format(Format, Format) ->
    [];
eval_format(?WAV_MIME, ?MP3_MIME) ->
    [fun audio_to_mp3/2
    ];
eval_format(CT, ?MP3_MIME) when ?WAV_COMPATIBLE(CT) ->
    [fun audio_to_mp3/2
    ];
eval_format(FromFormat, ToFormat) ->
    {'error', <<"invalid conversion requested: ", FromFormat/binary, " to: ", ToFormat/binary>>}.

-spec run_convert({'error', kz_term:ne_binary()} | audio_convert_funs()
                 ,kz_term:ne_binary()
                 ,kz_term:ne_binary()
...
-spec audio_to_mp3(kz_term:ne_binary(), map()) -> audio_converted().
audio_to_mp3(FromPath, Options) ->
    convert_file(?CONVERT_AUDIO_COMMAND, FromPath, <<".mp3">>, Options).

-spec convert_file(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), map()) -> audio_converted().
...
-spec validate_output(kz_term:ne_binary(), kz_term:ne_binary(), map()) ->
                             audio_converted().
validate_output(?MP3_MIME, Filename, #{<<"tmp_dir">> := TmpDir}) ->
    OutputFile = filename:join(TmpDir, <<(kz_binary:rand_hex(16))/binary, ".mp3">>),
    run_validate_command(?VALIDATE_AUDIO_COMMAND, Filename, OutputFile, TmpDir);
