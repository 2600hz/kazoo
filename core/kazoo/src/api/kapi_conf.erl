...
-spec get_account_id(api(terms())) -> api(binary()).
get_account_id(API) ->
    get_value(API, <<"Account-ID">>).

-spec get_action(api(terms())) -> api(binary()).
get_action(API) ->
    get_value(API, <<"Action">>).

-spec get_account_db(api(terms())) -> api(binary()).
get_account_db(API) ->
    get_value(API, <<"Account-DB">>).

-spec get_database(api(terms())) -> ne_binary().
get_database(API) ->
    get_value(API, <<"Database">>).

%% returns the public fields of the document
-spec get_doc(api(terms())) -> api(kz_json:object()).
get_doc(API) ->
    get_value(API, <<"Doc">>).

-spec get_id(api(terms())) -> api(binary()).
get_id(API) ->
    get_value(API, <<"ID">>).

%% returns the pvt_type field
-spec get_type(api(terms())) -> api(binary()).
get_type(API) ->
    get_value(API, <<"Type">>).

-spec get_is_soft_deleted(api(terms())) -> boolean().
get_is_soft_deleted(API) ->
    kz_util:is_true(get_value(API, <<"Is-Soft-Deleted">>)).

-spec get_value(api(terms()), ne_binary()) -> any().
...
-spec doc_update_v(api(terms())) -> boolean().
...
%% @end
%%--------------------------------------------------------------------
-spec doc_type_update(api(terms())) ->
                        {'ok', iolist()} |
                        {'error', string()}.
...
-spec doc_type_update_v(api(terms())) -> boolean().
...
-spec publish_doc_update(action(), ne_binary(), ne_binary(), ne_binary(), api(terms())) -> 'ok'.
-spec publish_doc_update(action(), ne_binary(), ne_binary(), ne_binary(), api(terms()), ne_binary()) -> 'ok'.
...
-spec publish_db_update(action(), ne_binary(), api(terms())) -> 'ok'.
-spec publish_db_update(action(), ne_binary(), api(terms()), ne_binary()) -> 'ok'.
...
-spec publish_doc_type_update(api(terms())) -> 'ok'.
-spec publish_doc_type_update(api(terms()), ne_binary()) -> 'ok'.
...
-spec doc_type_update_routing_key(api(terms()) | ne_binary()) -> ne_binary().
