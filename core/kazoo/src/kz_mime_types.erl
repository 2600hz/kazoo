...
from_extension(<<Extension/binary>>) ->
    case props:get_value(kz_term:to_list(Extension), mime_types()) of
        undefined ->
            kapps_config:get(?CONFIG_CAT, <<"default_content_type">>, <<"application/octet-stream">>);
        ContentType -> kz_term:to_binary(ContentType)
    end;
from_extension(Extension) ->
    from_extension(kz_term:to_binary(Extension)).
...
to_extension(<<CT/binary>>) ->
    case lists:keysearch(kz_term:to_list(CT), 2, mime_types()) of
        {value, {_, Extension}} -> kz_term:to_binary(Extension);
        false ->
            kapps_config:get(?CONFIG_CAT, <<"default_extension">>, <<"mp3">>)
    end;
to_extension(CT) ->
    to_extension(kz_term:to_binary(CT)).

mime_types() ->
    Default = filename:join(code:lib_dir(inets), "examples/server_root/conf/mime.types"),
    MimeTypesFile = kapps_config:get(?CONFIG_CAT, <<"mime_types_file">>, kz_term:to_binary(Default)),
