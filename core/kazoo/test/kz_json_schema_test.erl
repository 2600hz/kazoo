...
add_defaults_test() ->
    ...
    ?assertEqual(kz_json:get_value(<<"key1">>, WithDefaults), <<"value1">>),
    ?assertEqual(kz_json:get_value(<<"key2">>, WithDefaults), <<"default2">>).
...
add_sub_defaults_test() ->
    ...
    WithDefaults = kz_json_schema:add_defaults(JObj, SchemaJObj),
    ?assert(kz_json:is_json_object(kz_json:get_value(<<"caller_id">>, WithDefaults))),
    ?assertEqual(<<"external_cid">>, kz_json:get_value([<<"caller_id">>, <<"external">>, <<"name">>], WithDefaults)),
    ?assertEqual(<<"emer_default">>, kz_json:get_value([<<"caller_id">>, <<"emergency">>, <<"name">>], WithDefaults)).
...
add_sub_defaults_to_empty_test() ->
    ...
    ?assert(kz_json:is_json_object(kz_json:get_value(<<"caller_id">>, WithDefaults))),
    ?assertEqual(EmptyJObj, kz_json:get_value(<<"caller_id">>, WithDefaults)).
...
add_defaults_array_test() ->
    ...
    ?assertEqual(<<"auto">>, kz_json:get_value(<<"bypass_media">>, WithDefaults)),
    ?assertEqual([<<"PCMU">>], kz_json:get_value([<<"audio">>, <<"codecs">>], WithDefaults)).
...
add_sub_defaults_array_test() ->
    ...
    ?assertEqual('true', kz_json:get_value(<<"enabled">>, WithDefaults)),
    ?assertEqual('false', kz_json:get_value(<<"emergency">>, WithDefaults)),
    ?assertEqual(5, kz_json:get_value(<<"grace_period">>, WithDefaults)),
    ?assertEqual(50, kz_json:get_value(<<"weight_cost">>, WithDefaults)),
    ?assertEqual([], kz_json:get_value(<<"flags">>, WithDefaults)),

    Gateway = kz_json:get_value([<<"gateways">>, 1], WithDefaults),

    ?assert(kz_json:is_json_object(Gateway)),

    ?assertEqual('true', kz_json:get_value(<<"enabled">>, Gateway)),
    ?assertEqual(<<"+1">>, kz_json:get_value(<<"prefix">>, Gateway)),
    ?assertEqual([<<"PCMU">>, <<"PCMA">>], kz_json:get_value(<<"codecs">>, Gateway)),
    ?assertEqual(<<"auto">>, kz_json:get_value([<<"media">>, <<"fax_option">>], Gateway)),
    ?assertEqual([<<"PCMU">>, <<"PCMA">>], kz_json:get_value([<<"media">>, <<"codecs">>], Gateway)),
    ?assertEqual(kz_json:new(), kz_json:get_value(<<"custom_sip_headers">>, Gateway)),
    ?assertEqual(<<"auto">>, kz_json:get_value(<<"bypass_media">>, Gateway)),
    ?assertEqual(<<"sip">>, kz_json:get_value(<<"endpoint_type">>, Gateway)).
