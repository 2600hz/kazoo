...
update_routing_key(Req) ->
    update_routing_key(wh_json:get_value(<<"State">>, Req)
                      ,wh_json:get_value(<<"Presence-ID">>, Req)).
...
probe(Prop) when is_list(Prop) ->
    case probe_v(Prop) of
        'true' -> wh_api:build_message(Prop, ?PROBE_HEADERS, ?OPTIONAL_PROBE_HEADERS);
        'false' -> {'error', "Proplist failed validation for probe"}
    end;
probe(JObj) -> probe(wh_json:to_proplist(JObj)).

-spec probe_v(api_terms()) -> boolean().
probe_v(Prop) when is_list(Prop) ->
    wh_api:validate(Prop, ?PROBE_HEADERS, ?PROBE_VALUES, ?PROBE_TYPES);
probe_v(JObj) -> probe_v(wh_json:to_proplist(JObj)).
...
publish_probe(Req, ContentType) ->
    {'ok', Payload} = wh_api:prepare_api_payload(Req, ?PROBE_VALUES, fun ?MODULE:probe/1),
...
probe_routing_key(JObj) ->
    probe_routing_key(wh_json:get_value(<<"Event-Package">>, JObj)).
...
publish_mwi_update(Req, ContentType) ->
    {'ok', Payload} = wh_api:prepare_api_payload(Req, ?MWI_REQ_VALUES, fun ?MODULE:mwi_update/1),
...
mwi_update_routing_key(JObj) ->
    mwi_update_routing_key(wh_json:get_value(<<"To">>, JObj)).
...
publish_mwi_query(Req, ContentType) ->
    {'ok', Payload} = wh_api:prepare_api_payload(Req, ?MWI_QUERY_VALUES, fun ?MODULE:mwi_query/1),
...
mwi_query_routing_key(JObj) ->
    mwi_query_routing_key(wh_json:get_value(<<"Realm">>, JObj)).
...
publish_register_overwrite(Req, ContentType) when is_list(Req) ->
    {'ok', Payload} = wh_api:prepare_api_payload(Req, ?REGISTER_OVERWRITE_VALUES, fun ?MODULE:register_overwrite/1),
...
register_overwrite_routing_key(JObj) ->
    register_overwrite_routing_key(wh_json:get_value(<<"Realm">>, JObj)).
