    ...
    case GetFun(Paths, Req) of
        'undefined' ->
            case find_account_id(Req, GetFun) of
                'undefined' -> 'undefined';
                AccountId -> kzd_accounts:format_account_db(AccountId)
            end;
        ...
        ?MATCH_ACCOUNT_RAW(_)=Db -> kzd_accounts:format_account_db(Db);
        ?MATCH_ACCOUNT_UNENCODED(_)=Db -> kzd_accounts:format_account_db(Db);
        ?MATCH_ACCOUNT_ENCODED(_)=Db -> kzd_accounts:format_account_db(Db);
        ?MATCH_ACCOUNT_encoded(_)=Db -> kzd_accounts:format_account_db(Db);
        OtherDb -> OtherDb
    end.
...
maybe_strict_modb(Db, 'true') ->
    kzd_accounts:format_account_modb(Db, 'encoded');
maybe_strict_modb(Db, 'false') ->
    kzd_accounts:format_account_db(Db).
