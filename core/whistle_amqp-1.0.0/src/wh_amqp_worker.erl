...
handle_call({'publish', ReqProp, PublishFun}
    ...
    case publish_api(PublishFun, ReqProp) of
        'ok' ->
            lager:debug("published message ~s for ~p", [wh_api:msg_id(ReqProp), _Pid]),
            {'reply', 'ok', State};
        {'error', _E}=Err ->
            lager:error("failed to publish message ~s for ~p: ~p", [wh_api:msg_id(ReqProp), _Pid, _E]),
            {'reply', Err, State}
    end;
handle_call({'publish', ReqProp, PublishFun}
           ,{Pid, _}=From
...
%% @end
%%--------------------------------------------------------------------
handle_event(JObj, #state{client_pid=Pid
                         ,client_from='relay'
                         }) ->
    whapps_call_command:relay_event(Pid, JObj),
    lager:debug("relayed payload to ~p", [Pid]),
    'ignore';
