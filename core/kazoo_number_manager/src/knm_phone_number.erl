...
save(PN=#knm_phone_number{dry_run='true'}) ->
...
delete(PN=#knm_phone_number{dry_run='true'}) ->
    lager:debug("dry_run-ing btw"),
    PN;
delete(PN) ->
...
set_modified(PN=#knm_phone_number{modified = undefined}, undefined) ->
    ?DIRTY(PN#knm_phone_number{modified = kz_util:current_tstamp()});
...
set_created(PN=#knm_phone_number{created = undefined}, undefined) ->
    ?DIRTY(PN#knm_phone_number{created = kz_util:current_tstamp()});
...
list_attachments(PN, AuthBy) ->
    ...
    case state(PN) =:= ?NUMBER_STATE_PORT_IN
        andalso is_admin_or_in_account_hierarchy(AuthBy, AssignedTo)
    of
