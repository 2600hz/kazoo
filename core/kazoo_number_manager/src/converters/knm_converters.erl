...
-spec normalize(ne_binary(), api(binary()), kz_json:object()) ->
                       ne_binary().
...
%%--------------------------------------------------------------------
-spec to_db(<<_:40,_:_*8>>) -> api(binary()).
...
-spec classify(ne_binary()) -> api(binary()).
classify(Number) ->
    Num = normalize(Number),
...
%% @end
%%--------------------------------------------------------------------
-spec classify_number(ne_binary(), kz_proplist() | ne_binary()) -> api(binary()).
classify_number(Num, []) ->
