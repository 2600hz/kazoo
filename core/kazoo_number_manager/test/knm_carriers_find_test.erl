...
info_test_() ->
    ...
    SetMaster = sets:from_list([?NUMBER_STATE_IN_SERVICE
                               ,?NUMBER_STATE_RESERVED
                               ,?NUMBER_STATE_AVAILABLE
                               ]),
    SetReseller = sets:from_list([?NUMBER_STATE_IN_SERVICE
                                 ,?NUMBER_STATE_RESERVED
                                 ]),
    ...
    ,?_assert(sets:is_subset(SetMaster, sets:from_list(kz_json:get_value(<<"usable_creation_states">>, InfoJObj4))))
    ,?_assert(SetReseller =:= sets:from_list(kz_json:get_value(<<"usable_creation_states">>, InfoJObj5)))
    ].
