...
db_classification(<<"numbers%2f", _Prefix:5/binary>>) -> 'numbers';
db_classification(<<"account/", _AccountId:34/binary, "-", Suffix/binary>>) when Suffix =:= ?WH_RATES_DB -> 'ratedeck';
db_classification(<<"account%2F", _AccountId:38/binary, "-", Suffix/binary>>) when Suffix =:= ?WH_RATES_DB -> 'ratedeck';
db_classification(<<"account%2f", _AccountId:38/binary, "-", Suffix/binary>>) when Suffix =:= ?WH_RATES_DB -> 'ratedeck';
