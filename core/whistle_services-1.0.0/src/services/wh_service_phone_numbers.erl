    ...
    case catch wnm_number:get(DID) of
        #number{module_name=Module}=Number ->
			case catch Module:is_number_billable(Number) of
				'true' -> lager:debug("number ~s is billable: ~s", [DID,Module]),'true';
				'false' -> lager:debug("number ~s is not billable: ~s", [DID,Module]),'false';
				Err -> lager:debug("number ~s is not billable due to provider ~s error: ~p", [DID,Module,Err]),'false'
			end;
		Other -> lager:debug("number ~s is not billable due to number error: ~p", [DID,Other]),
				 'false'
    end.
