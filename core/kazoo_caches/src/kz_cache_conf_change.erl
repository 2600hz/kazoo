...
-spec handle_change(atom() | [atom()], kapi_conf:doc()) -> 'ok'.
...
-spec exec_bindings(kz_term:ne_binary() | atom(), kz_json:object()) -> 'ok'.
exec_bindings(Name, JObj) ->
    ...
    RK = kz_binary:join([<<"kapi.conf">>, kz_term:to_binary(Name), Db, Type, Event, Id], <<".">>),
