...
should_block_anonymous(JObj) ->
    ...
    case {kz_util:is_true(ShouldBlock), is_anonymous(JObj)} of
        {'true', 'true'} ->
...
is_anonymous(JObj) ->
    IsPrivacyNumber = kz_util:is_true(get_value(<<"Caller-Privacy-Number">>, JObj, 'false')),
    IsPrivacyName = kz_util:is_true(get_value(<<"Caller-Privacy-Name">>, JObj, 'false')),
...
caller_screen_bit(JObj) -> kz_util:is_true(get_value(?CALLER_SCREEN_BIT, JObj, 'false')).

-spec caller_privacy_name(kz_json:object()) -> boolean().
caller_privacy_name(JObj) -> kz_util:is_true(get_value(?CALLER_PRIVACY_NAME, JObj, 'false')).

-spec caller_privacy_number(kz_json:object()) -> boolean().
caller_privacy_number(JObj) -> kz_util:is_true(get_value(?CALLER_PRIVACY_NUMBER, JObj, 'false')).
