...
do_get(Nums, Options) ->
    ...
    knm_pipe:do(fun knm_phone_number:fetch_pipe/1
               ,knm_pipe:new(Options
                            ,lists:usort(Yes)
                            ,lists:usort(No)
                            )
               ).
...
create(Nums, Opts0) ->
    lager:debug("try creating ~b numbers", [length(Nums)]),
...
%% FIXME: opaque
%% FIXME: `services' is for auth account not the assigned account
...
-spec maybe_call_callbacks(knm_pipe:collection()) -> knm_pipe:collection().
-ifdef(TEST).
maybe_call_callbacks(Collection) -> knm_pipe:set_succeeded(Collection, knm_pipe:todo(Collection)).
-else.
maybe_call_callbacks(Collection) ->
    Todo = knm_pipe:todo(Collection),
    PNsWithCallBacks = [PN || PN <- Todo,
                              kz_term:is_not_empty(knm_phone_number:on_success(PN))
                       ],
    case knm_options:dry_run(knm_pipe:options(Collection))
        orelse kz_term:is_empty(PNsWithCallBacks)
    of
        'true' ->
            knm_pipe:set_succeeded(Collection, Todo);
        'false' ->
            _Pid = kz_process:spawn(fun call_callbacks/1, [PNsWithCallBacks]),
            lager:debug("running after save callbacks for ~b number(s) in ~p"
                       ,[_Pid, length(PNsWithCallBacks)]
                       ),
            knm_pipe:set_succeeded(Collection, Todo)
    end.

-spec call_callbacks(knm_phone_number:records() | knm_phone_number:record()) -> 'ok'.
call_callbacks(PNs) when is_list(PNs) ->
    plists:foreach(fun call_callbacks/1, PNs);
call_callbacks(PN) ->
    CBs = knm_phone_number:on_success(PN),
    plists:foreach(fun({CB, Args}) -> call_callback(PN, CB, Args) end, CBs).

-spec call_callback(knm_phone_number:record(), knm_phone_number:callback(), list()) -> any().
call_callback(PN, CB, Args) ->
    _Number = knm_phone_number:number(PN),
    lager:debug("applying ~p on number ~s", [CB, _Number]),
    try erlang:apply(CB, [PN | Args])
    catch
        _T:_E ->
            lager:debug("number ~s callback ~p failed: ~p"
                       ,[_Number, CB, {_T, _E}]
                       )
    end.
-endif.
