...
-spec format_numbers_resp(kz_term:ne_binary(), kz_json:objects()) -> knm_search:results().
...
acquire_number(PN) ->
    ...
    update_doc(PN, [{kzd_phone_numbers:pvt_state_path(), knm_phone_number:state(PN)}
                   ,{kzd_phone_numbers:pvt_assigned_to_path(), knm_phone_number:assigned_to(PN)}
                   ]).
...
disconnect_number(PN) ->
    ...
    update_doc(PN, [{kzd_phone_numbers:pvt_state_path(), ?NUMBER_STATE_RELEASED}
