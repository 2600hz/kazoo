...
create_new_number_rename_carrier() ->
    ...
    [PN1] = knm_pipe:succeeded(knm_ops:create([?TEST_CREATE_NUM], Options)),

    WrongCarrier = kz_json:from_list([{?FEATURE_RENAME_CARRIER, <<"wrong_carrier">>}]),
    {'ok', [], [{?TEST_CREATE_NUM, Error2}]} = knm_numbers:create(?TEST_CREATE_NUM, props:set_value('public_fields', WrongCarrier, Options)),

    {'ok', [], [{?TEST_CREATE_NUM, Error3}]} = knm_numbers:create(?TEST_CREATE_NUM, props:set_value('auth_by', ?RESELLER_ACCOUNT_ID, Options)),
...
rename_carrier() ->
    ...
    ,{"Verify renamed failed"
     ,?_assertEqual(<<"'blabla' is not known by the system">>, kz_json:get_value(<<"cause">>, kz_maps:get(['failed', ?TEST_TELNYX_NUM], E3, kz_json:new())))
     }
...
rename_from_local() ->
    ...
    [PN1] = knm_pipe:succeeded(knm_ops:get([?TEST_IN_SERVICE_NUM], Options)),
...
rename_to_local_with_external_features() ->
    ...
    [PN1] = knm_pipe:succeeded(knm_ops:get([?TEST_VITELITY_NUM], Options)),
    JObj1 = kz_json:from_list([{?FEATURE_RENAME_CARRIER, <<"local">>}]),
