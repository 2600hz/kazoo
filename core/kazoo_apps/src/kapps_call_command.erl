...
-spec do_store_file(pos_integer(), kz_timeout(), kz_json:object()
                   ,ne_binary(), kapps_call:call()) ->
          'ok' | {'error', any()}.
do_store_file(Tries, Timeout, API, Msg, Call) ->
    case kz_amqp_worker:call(API, fun kapi_switch:publish_command/1, fun kapi_switch:fs_reply_v/1, Timeout) of
        ...
        {'returned', _JObj, _Basic} ->
            ...
            retry_store_file(Tries, Timeout, API, Msg, Error, kapps_call:exec(Funs, Call));
        {'timeout', _JObj} ->
...
-spec retry_store_file(integer(), kz_timeout(), kz_json:object()
