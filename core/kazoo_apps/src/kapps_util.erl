...
replicate_from_account(AccountDb, AccountDb, _) ->
    ...
    {'error', 'matching_dbs'};
replicate_from_account(AccountDb, TargetDb, FilterDoc) ->
    ReplicateProps = [{<<"source">>, kz_account:format_id(AccountDb, ?REPLICATE_ENCODING)}
                      ,{<<"target">>, TargetDb}
                      ,{<<"filter">>, FilterDoc}
                      ,{<<"create_target">>, 'true'}
                     ],
...
get_master_account_db() ->
    case get_master_account_id() of
        {'error', _}=E -> E;
        {'ok', AccountId} ->
            {'ok', kz_account:format_id(AccountId, 'encoded')}
    end.
...
get_all_accounts(Encoding) ->
    ...
    [kz_account:format_id(Db, Encoding)
...
format_db(Db, Encoding) ->
    Fs =
        [{fun is_account_db/1, fun kz_account:format_id/2}
         ,{fun is_account_mod/1, fun kz_account:format_modb/2}
        ],
...
get_all_account_mods(Encoding) ->
    {'ok', Databases} = kz_datamgr:db_info(),
    [kz_account:format_modb(Db, Encoding)
     || Db <- Databases,
        is_account_mod(Db)
    ].
...
get_account_mods(AccountId, Encoding) ->
    ...
    [kz_account:format_id(MOD, Encoding)
     || MOD <- MODs,
        is_account_mod(MOD),
