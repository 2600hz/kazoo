...
put_attachment(Params, DbName, DocId, AName, Contents, _Options) ->
    ...
    FilePath = wh_util:to_list(list_to_binary([Path, "/", DbName, "/", DocId, "_", AName])),
    Config = kz_aws_s3:new(wh_util:to_list(Key), wh_util:to_list(Secret)),
    case kz_aws_s3:put_object(wh_util:to_list(Bucket), FilePath, Contents, Config) of
        {'ok', Props} ->
...
fetch_attachment(HandlerProps, DbName, DocId, AName) ->
    case wh_json:get_value(<<"S3">>, HandlerProps) of
        'undefined' -> {'error', 'invalid_data'};
        S3 ->
            {Key, Secret, Bucket, Path} = binary_to_term(base64:decode(S3)),
            FilePath = wh_util:to_list(list_to_binary([Path, "/", DbName, "/", DocId, "_", AName])),
            Config = kz_aws_s3:new(wh_util:to_list(Key), wh_util:to_list(Secret)),
            case kz_aws_s3:get_object(wh_util:to_list(Bucket), FilePath, Config) of
