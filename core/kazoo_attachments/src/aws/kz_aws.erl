...
aws_region_from_host(Host) ->
    case string:tokens(Host, ".") of
        ...
        %% format
        ["streams", "dynamodb", Value | _Rest] -> Value;
        [_, Value, _, _ | _Rest] -> Value;
        _ -> "us-east-1"
    end.
    ...
    Response =
        case Method of
            'get' ->
                ...
                kz_aws_httpc:request(Req, 'get', Headers, <<>>, timeout(Config), Config);
            _ ->
...
-spec param_list([tuple()], tuple() | string()) -> [tuple()].
param_list([], _Key) -> [];
...
-spec format_timestamp(calendar:datetime()) -> nonempty_string().
...
-spec update_config(aws_config()) -> {'ok', aws_config()} | {'error', any()}.
...
update_config(#aws_config{} = Config) ->
    ...
    case get_metadata_credentials(Config) of
        ...
        {'ok', Credentials} ->
            NewConfig =
                Config#aws_config{access_key_id = Credentials#metadata_credentials.access_key_id
...
-spec http_body({'ok', tuple()} | {'error', any()})
...
-spec http_headers_body({'ok', tuple()} | {'error', any()})
