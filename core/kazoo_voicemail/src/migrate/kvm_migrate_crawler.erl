...
init(Pid) ->
    ...
    case kapps_util:get_all_accounts('raw') of
        [] ->
            ...
            {'stop', #state{}};
        Ids ->
            ?WARNING(":: BEGINNING MIGRATING VOICEMAIL FOR ~b ACCOUNTS", [length(Ids)]),
...
maybe_spawn_worker(#state{account_ids = AccountIds
    ...
    ?WARNING(":: all voicemails in retention duration are migrated, beginning a migrating older voicemails", []),
