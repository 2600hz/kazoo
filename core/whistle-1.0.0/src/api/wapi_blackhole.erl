...
get_req_v(Prop) when is_list(Prop) ->
    wh_api:validate(Prop, ?BLACKHOLE_GET_REQ_HEADERS, ?BLACKHOLE_GET_REQ_VALUES, ?BLACKHOLE_TYPES);
get_req_v(JObj) ->
...
get_resp_v(Prop) when is_list(Prop) ->
    wh_api:validate(Prop, ?BLACKHOLE_GET_RESP_HEADERS, ?BLACKHOLE_GET_RESP_VALUES, ?BLACKHOLE_TYPES);
get_resp_v(JObj) ->
...
publish_get_req(Api, ContentType) ->
    {'ok', Payload} = wh_api:prepare_api_payload(Api, ?BLACKHOLE_GET_REQ_VALUES, fun ?MODULE:get_req/1),
...
publish_get_resp(RespQ, Api, ContentType) ->
    ...
    {'ok', Payload} = wh_api:prepare_api_payload(Api, ?BLACKHOLE_GET_RESP_VALUES, PrepareOptions),
