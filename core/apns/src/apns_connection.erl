...
handle_cast(#apns_msg{device_token = DeviceToken
                     ,expiry = Expiry
                     ,id = Id
                     ,priority = Priority
                     } = Msg,
            #state{out_socket = Socket
                  ,queue = Queue
                  } = State) ->
...
handle_info({'ssl_closed', SslSocket}, State = #state{in_socket = SslSocket
    ...
    _Timer =
        erlang:send_after(Connection#apns_connection.feedback_timeout
                         ,self()
                         ,'reconnect'
                         ),
...
build_payload(Params, Extra, Content_Available) ->
    kz_json:encode(
      kz_json:from_list(
        [{<<"aps">>, do_build_payload(Params, Content_Available)}
         |Extra
        ]
       )
     ).
