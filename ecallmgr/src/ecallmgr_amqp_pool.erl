...
-export([authn_req/1, authn_req/2
         ...
         ,get_req/1, get_req/2
         ,set_req/1, set_req/2
         ,originate_ready/2, originate_ready/3
        ]).
...
-spec authn_req/1 :: (api_terms()) -> {'ok', wh_json:json_object()} |
                                      {'error', _}.
-spec authn_req/2 :: (api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                     {'error', _}.
authn_req(AuthnReq) ->
...
-spec authz_req/1 :: (api_terms()) -> {'ok', wh_json:json_object()} |
                                      {'error', _}.
-spec authz_req/2 :: (api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                     {'error', _}.
authz_req(AuthzReq) ->
...
-spec route_req/1 :: (api_terms()) -> {'ok', wh_json:json_object()} |
                                      {'error', _}.
-spec route_req/2 :: (api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                     {'error', _}.
route_req(RouteReq) ->
...
-spec reg_query/1 :: (api_terms()) -> {'ok', wh_json:json_object()} |
                                      {'error', _}.
-spec reg_query/2 :: (api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                     {'error', _}.
reg_query(RegReq) ->
...
-spec media_req/1 :: (api_terms()) -> {'ok', wh_json:json_object()} |
                                      {'error', _}.
-spec media_req/2 :: (api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                     {'error', _}.
media_req(MediaReq) ->
...
-spec get_req/1 :: (api_terms()) -> {'ok', wh_json:json_object()} |
                                    {'error', _}.
-spec get_req/2 :: (api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                   {'error', _}.
get_req(GetReq) ->
...
-spec set_req/1 :: (api_terms()) -> {'ok', wh_json:json_object()} |
                                    {'error', _}.
-spec set_req/2 :: (api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                   {'error', _}.
set_req(GetReq) ->
...
-spec originate_ready/2 :: (ne_binary(), api_terms()) -> {'ok', wh_json:json_object()} |
                                                         {'error', _}.
-spec originate_ready/3 :: (ne_binary(), api_terms(), pos_integer()) -> {'ok', wh_json:json_object()} |
                                                                        {'error', _}.
originate_ready(ServerId, Req) ->
    originate_ready(ServerId, Req, ?DEFAULT_TIMEOUT).
originate_ready(ServerId, Req, Timeout) ->
    send_request(Req, Timeout
                 ,fun(Api) -> wapi_dialplan:publish_originate_ready(ServerId, Api) end
