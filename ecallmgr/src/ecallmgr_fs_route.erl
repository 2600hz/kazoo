...
process_route_req(Node, FSID, CallId, Props) ->
    ...
    case ecallmgr_fs_nodes:channel_node(CallId) of
        ...
        {ok, Node} ->
            case props:get_value(<<"variable_recovered">>, Props) of
                undefined ->
                    lager:debug("channel already exists on ~s, but not a recovered one, so search", [Node]),
                    search_for_route(Node, FSID, CallId, Props);
                _Recovered ->
                    lager:debug("channel already exists on ~s, recovered: ~p", [Node, _Recovered]),
