...
-spec authorize/3 :: (FSID, CallID, FSData) -> tuple(ok, pid()) when
      FSID :: binary(),
      CallID :: binary(),
      FSData :: proplist().
...
-spec is_authorized/1 :: (Pid) -> boolean() when
      Pid :: pid() | undefined.
...
init_authorize(Parent, FSID, CallID, FSData) ->
    ...
    {ok, RespProp} = ecallmgr_amqp_pool:authz_req(ReqProp),

    authorize_loop(whistle_util:is_true(wh_json:get_value(<<"Is-Authorized">>, RespProp))).

authorize_loop(IsAuth) ->
    ?LOG("Is Authorized: ~s", [IsAuth]),
    receive
	{is_authorized, From, Ref} ->
	    From ! {is_authorized, Ref, IsAuth}
    end.
