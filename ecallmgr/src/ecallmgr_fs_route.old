...
send_request(Host, JSON) ->
    amqp_util_old:callmgr_publish(Host, JSON, <<"application/json">>, ?KEY_ROUTE_REQ).
...
lookup_reg(AmqpHost, Domain, User) ->
    Self = self(),
    spawn(fun() ->
		  Q = amqp_util_old:new_targeted_queue(AmqpHost, <<>>),
		  amqp_util_old:bind_q_to_targeted(AmqpHost, Q),
		  ...
		  amqp_util_old:broadcast_publish(AmqpHost, JSON, <<"application/json">>),
